...
export function StudyAdvice({ practiceSets }: StudyAdviceProps) {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-5" data-oid="-loc:9k">
      <h3 className="font-medium text-slate-800 dark:text-white text-lg mb-4" data-oid="87gll8p">Personalized Advice</h3>
      
      <div className="space-y-4" data-oid="m_3a4kr">
        {advice.map((item, index) =>
        <div key={index} className="flex bg-gradient-to-r from-slate-50 to-white dark:from-slate-800/50 dark:to-slate-800 rounded-lg p-4 shadow-inner" data-oid="5k96g1f">
            <div className="flex-shrink-0 flex items-center justify-center h-10 w-10 rounded-full bg-indigo-100 dark:bg-indigo-900/30 text-lg" data-oid="eo3sjbw">
              {item.icon}
            </div>
            <div className="ml-4" data-oid="x-h2tt0">
              <h4 className="text-base font-medium text-slate-900 dark:text-white" data-oid="y1lct:i">{item.title}</h4>
              <p className="mt-1 text-sm text-slate-500 dark:text-slate-400" data-oid="6nawpe1">{item.description}</p>
              <a
              href={item.actionLink}
              className="inline-flex items-center mt-2 text-sm font-medium text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300" data-oid="-oa7vfq">

                {item.actionText}
                <svg className="ml-1 h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" data-oid="tgt41vu">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7l5 5m0 0l-5 5m5-5H6" data-oid="z28box0" />
                </svg>
              </a>
            </div>
          </div>
