...
export function SpiderChartView({ practiceSets, onSelectSet, selectedSetId }: QuestionViewProps) {
  ...
  return (
    <div className="border border-slate-200 dark:border-slate-700 rounded-xl p-4 shadow-sm">
      <h3 className="text-xl font-bold mb-6 text-center">13. Spider/Radar Chart View</h3>
      ...
      <div className="mb-6 flex flex-wrap gap-4 justify-center">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-lg p-2 shadow-sm">
          <label className="block text-sm text-slate-500 dark:text-slate-400 mb-1">Metric</label>
          <select 
            className="bg-slate-50 dark:bg-slate-700 text-slate-900 dark:text-white rounded-md px-2 py-1 text-sm border border-slate-200 dark:border-slate-600 w-full"
            value={metricType}
            onChange={(e) => setMetricType(e.target.value as 'accuracy' | 'count' | 'time')}
          >
            <option value="accuracy">Accuracy (%)</option>
            <option value="count">Question Count</option>
            <option value="time">Average Time (sec)</option>
          </select>
        </div>
      </div>
      
      {/* Spider Chart */}
      <div className="bg-white dark:bg-slate-800 rounded-lg p-4 shadow-sm mb-6 overflow-hidden">
        <svg 
          width="100%" 
          height="auto" 
        ...
        >
          ...
          {/* Axes */}
          {axes.map((axis, i) => (
            <g key={`axis-${i}`} className="cursor-pointer" onClick={() => handleTopicSelect(axis.topic)}>
              <line
                x1={axis.line.x1}
                y1={axis.line.y1}
                x2={axis.line.x2}
          ...
          <g>
            ...
            {chartData.points.map((point, i) => (
              <g key={`point-${i}`} className="cursor-pointer" onClick={() => handleTopicSelect(topicMetrics[i].topic)}>
                <circle
                  cx={point.x}
                  cy={point.y}
                  r="4"
          ...
          {/* Legend */}
          <g transform={`translate(${chartSize - 120}, 20)`}>
            <rect width="100" height="24" fill="white" fillOpacity="0.8" rx="4" ry="4" />
            <text x="10" y="16" fontSize="12" fill="#334155">
              {getMetricLabel()}
            </text>
          </g>
        </svg>
            ...
            <tbody>
              {topicMetrics.map((metric, i) => (
                <tr 
                  ...
                  onClick={() => handleTopicSelect(metric.topic)}
                >
                  <td className="py-2">{metric.topic}</td>
                  ...
                  <td className="py-2 text-right">{metric.time.toFixed(1)}s</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="mt-4 text-sm text-slate-500 dark:text-slate-400 text-center">
        <p>Click on any axis or data point to view details for that topic.</p>
      </div>
    </div>
  )
}
