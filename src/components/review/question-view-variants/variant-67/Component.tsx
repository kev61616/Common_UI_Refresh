...
// Space-themed grid with galaxies, nebulae, and celestial objects representing knowledge domains
 ...
 */
export function CosmicMatrixView({ practiceSets, onSelectSet, selectedSetId }: QuestionViewProps) {
  ...
  return (
    <div className="border border-slate-200 dark:border-slate-700 rounded-xl p-4 shadow-sm" data-oid="5d1x.-o">
      <h3 className="text-xl font-bold mb-6 text-center" data-oid="o20b7-0">67. Cosmic Matrix</h3>
      
      {/* Subject Filter */}
      <div className="mb-6" data-oid="nfae:l4">
        <div className="flex space-x-2 overflow-x-auto pb-2" data-oid="kp9y6gk">
          <button
      ...
      {/* Matrix Grid */}
      <div className="overflow-x-auto mb-6" data-oid="pc5ptqu">
        <table className="w-full border-collapse" data-oid="o9md.9.">
          <thead data-oid="se0yole">
            <tr data-oid="ou_6__v">
              <th className="p-3 text-left font-medium text-sm text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-800/50 border-b dark:border-slate-700" data-oid="qbq0k56">
                Topic / Difficulty
              </th>
              ...
              <th
                ...
                'text-rose-600 dark:text-rose-400 bg-rose-50/50 dark:bg-rose-900/10'}`
                  ...
                  {difficulty}
                  <div className="text-xs font-normal mt-1 text-slate-500 dark:text-slate-400" data-oid="aorx-ob">
                    {difficultyTotals.find((d) => d.difficulty === difficulty)?.count || 0} questions
                  </div>
                </th>
              )}
              
              <th className="p-3 text-center font-medium text-sm text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-800/50 border-b dark:border-slate-700" data-oid="-92cx1q">
                Total
                <div className="text-xs font-normal mt-1" data-oid="hg9xzux">
          ...
          <tbody data-oid="8p4gbj-">
            {grid.map((row) =>
            <tr key={row.topic} data-oid="gapf-p2">
                <td className="p-3 text-sm font-medium border-b dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50" data-oid="kat-hji">
                  {row.topic}
                  <div className="text-xs font-normal text-slate-500 dark:text-slate-400" data-oid="_9w0oai">
              ...
              <td
                ...
                ''}`
                    ...
                    <div className={`h-24 p-3 cursor-pointer transition-all hover:shadow-md ${getCellColor(cell.accuracy, cell.count)}`} data-oid="bunzs-v">
                      <div className="flex flex-col h-full justify-between" data-oid="2ejm_6u">
                        <div className={`text-lg font-bold ${getTextColor(cell.accuracy, cell.count)}`} data-oid="x-w4fh9">
                          {cell.count > 0 ? `${cell.accuracy}%` : '-'}
                        </div>
                        
                        <div data-oid="p3372pi">
                          <div className="text-xs text-slate-500 dark:text-slate-400" data-oid="i-l8zed">
                            {cell.count} question{cell.count !== 1 ? 's' : ''}
                          </div>
                          
                          {cell.count > 0 &&
                      <div className="w-full bg-slate-200 dark:bg-slate-700 h-1 mt-1 rounded-full overflow-hidden" data-oid="zancgq3">
                              <div
                          ...
                          style={{ width: `${cell.accuracy}%` }} data-oid="mp4x.rz">
                        </div>
                            </div>
                      }
                        </div>
                ...
                <td className="p-3 text-center border-b dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50" data-oid="eyayo_2">
                ...
                topicTotals.find((t) => t.topic === row.topic)?.count || 0)}`
                  ...
                  <div className="w-full bg-slate-200 dark:bg-slate-700 h-1 mt-1 rounded-full overflow-hidden" data-oid="8ltbr2m">
                    <div
                    className={`h-full rounded-full ${
                    ...
                    style={{ width: `${topicTotals.find((t) => t.topic === row.topic)?.accuracy || 0}%` }} data-oid="cnyyc4:">
                  </div>
                  </div>
                </td>
              </tr>
            ...
            {/* Grand Total Row */}
            <tr data-oid="w7w5axz">
              <td className="p-3 text-sm font-medium border-b dark:border-slate-700 bg-slate-100 dark:bg-slate-800" data-oid="m441.a:">
                All Topics
              </td>
              ...
              <td
                ...
                className="p-3 text-center border-b dark:border-slate-700 bg-slate-100 dark:bg-slate-800" data-oid="rnwbxb.">

                  <div className={`text-lg font-bold ${getTextColor(total.accuracy, total.count)}`} data-oid="-zd-yim">
                    {total.accuracy}%
                  </div>
                  
                  <div className="w-full bg-slate-200 dark:bg-slate-700 h-1 mt-1 rounded-full overflow-hidden" data-oid="_6-wk5t">
                    <div
                    className={`h-full rounded-full ${
                    ...
                    style={{ width: `${total.accuracy}%` }} data-oid="l:wkwrz">
                  </div>
                  </div>
                </td>
              )}
              
              <td className="p-3 text-center border-b dark:border-slate-700 bg-indigo-50 dark:bg-indigo-900/20" data-oid=".i.t4h6">
                <div className={`text-lg font-bold ${getTextColor(grandTotal.accuracy, grandTotal.count)}`} data-oid="qwz3n.0">
                  {grandTotal.accuracy}%
                </div>
                
                <div className="w-full bg-slate-200 dark:bg-slate-700 h-1 mt-1 rounded-full overflow-hidden" data-oid="fp:sn0h">
                  <div
                    className={`h-full rounded-full ${
                    ...
                    style={{ width: `${grandTotal.accuracy}%` }} data-oid="4uagb8:">
                  </div>
                </div>
              </td>
            </tr>
      ...
      {selectedCell &&
      <div className="mt-6 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-200 dark:border-slate-700" data-oid="yp7.rjo">
          <div className="flex justify-between items-center mb-3" data-oid="hvr:1k8">
            <h4 className="font-semibold" data-oid="ovhd64_">
              {selectedCell.topic} - {selectedCell.difficulty} Questions
            </h4>
            <button
            onClick={() => setSelectedCell(null)}
            className="text-xs px-2 py-1 bg-white dark:bg-slate-700 rounded border border-slate-200 dark:border-slate-600" data-oid="w:ivn0g">

              Close
            </button>
          </div>
          
          <div className="space-y-2 max-h-[250px] overflow-y-auto" data-oid="v-s7kq.">
          ...
          <div
            ...
            'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50'}`
                ...
                <div data-oid="ji7wb:t">
                  <div className="font-medium text-sm flex items-center" data-oid="7rppdos">
                ...
                'bg-red-500 dark:bg-red-400'}`
                  ...
                  <div className="text-xs text-slate-500 dark:text-slate-400" data-oid=":2mgsw9">
                    From {question.setType} • {question.setSubject}
                  </div>
                </div>
                <div className="text-right text-xs" data-oid="wsy3jsb">
          ...
          <div className="py-6 text-center text-slate-500 dark:text-slate-400" data-oid="mzjaloc">
                <p data-oid="7zmlle-">No questions available for this combination.</p>
              </div>
          }
          </div>
        </div>
