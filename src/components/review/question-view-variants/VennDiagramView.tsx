...
export function VennDiagramView({ practiceSets, onSelectSet, selectedSetId }: QuestionViewProps) {
  ...
  return (
    <div className="border border-slate-200 dark:border-slate-700 rounded-xl p-4 shadow-sm">
      <h3 className="text-xl font-bold mb-6 text-center">9. Venn Diagram View</h3>
      ...
      <div className="bg-slate-50 dark:bg-slate-800/50 p-3 rounded-lg mb-6 text-sm text-slate-600 dark:text-slate-300">
        <p>This diagram shows relationships between {view}. The size of each circle represents the number of questions, and overlapping areas show common elements.</p>
      </div>
      
      {/* Venn Diagram */}
      <div className="aspect-video bg-white dark:bg-slate-800 rounded-lg shadow-sm overflow-hidden relative mb-4">
        <svg className="w-full h-full" viewBox="0 0 100 100" preserveAspectRatio="xMidYMid meet">
          ...
          {keys.map((key, i) => {
            ...
            return (
              <g key={i} onClick={() => handleRegionClick(key)} className="cursor-pointer">
                <circle
                  cx={coord.cx}
                  cy={coord.cy}
                  r={`${adjustedRadius}%`}
                ...
                <text
                  ...
                  className="text-sm font-medium fill-slate-900 dark:fill-white pointer-events-none"
                >
                  {key}
                </text>
      ...
      {/* Details section */}
      <div>
        <h4 className="text-lg font-semibold mb-3">Question Distribution</h4>
            ...
            return (
              ...
              >
                ...
                <div className="flex justify-between text-sm">
                  <span>Questions: {questions ? questions.length : 0}</span>
