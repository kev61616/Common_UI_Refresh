        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pb-4">
          {masteryLevels.map(level => {
            ...
            return (
              <div 
                key={level.id}
                className={`${level.color} border ${level.borderColor} rounded-lg shadow-sm overflow-hidden transition-all duration-300`}
              >
                {/* Column header */}
                <div 
                  className={`${level.headerBg} cursor-pointer p-4 flex items-center justify-between`}
                  onClick={() => toggleColumnExpansion(level.id)}
                >
                ...
                {/* Column content */}
                <div className={`p-3 transition-all duration-300 ${isExpanded ? 'max-h-[800px]' : 'max-h-[150px]'} overflow-hidden`}>
                  {!isExpanded ? (
