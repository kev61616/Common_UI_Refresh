...
export function ConceptMapView({ practiceSets, onSelectSet, selectedSetId }: QuestionViewProps) {
  ...
  return (
    <div className="border border-slate-200 dark:border-slate-700 rounded-xl p-4 shadow-sm" data-oid="d12c7-p">
      <h3 className="text-xl font-bold mb-6 text-center" data-oid="hx2jtvt">6. Concept Map View</h3>
      
      {/* Controls */}
      <div className="flex justify-between mb-6" data-oid="5701dcu">
        {/* Subject filter */}
        <div className="flex space-x-2" data-oid="blc_v9_">
          <button
        ...
        {/* Zoom controls */}
        <div className="flex items-center space-x-2" data-oid="h1jp3dm">
          <button
            onClick={handleZoomOut}
            className="w-8 h-8 flex items-center justify-center bg-white rounded-full border border-slate-200 text-slate-700 hover:bg-slate-50 dark:bg-slate-800 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-700" data-oid="5gug:p4">

            <span className="text-lg" data-oid="rpvsgyh">-</span>
          </button>
          <div className="text-sm text-slate-500 dark:text-slate-400" data-oid="-pnv8t5">
            {Math.round(zoomLevel * 100)}%
          </div>
          <button
            onClick={handleZoomIn}
            className="w-8 h-8 flex items-center justify-center bg-white rounded-full border border-slate-200 text-slate-700 hover:bg-slate-50 dark:bg-slate-800 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-700" data-oid="ypgqyyk">

            <span className="text-lg" data-oid="frg7q:r">+</span>
          </button>
        </div>
      </div>
      
      {/* Concept Map Visualization */}
      <div className="relative overflow-hidden border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 h-[500px]" data-oid="-fqlwj6">
        {/* SVG Canvas */}
        <div
          ref={canvasRef}
          ...
          <svg width="100%" height="100%" viewBox="0 0 800 500" preserveAspectRatio="xMidYMid meet" data-oid="wy7qwds">
            {/* Links */}
            <g data-oid="wf9jw0s">
              {links.map((link, i) =>
              <line
                ...
                strokeDasharray={link.type === 'topic-topic' ? "5,5" : ""} data-oid="q4p8-c1" />
            ...
            <g
              ...
              className="cursor-pointer" data-oid="1c.6w40">

                <circle
                r={node.radius}
                className={`${getNodeColor(node)} border transition-all ${getSelectedClass(node)}`} data-oid=".owzsf." />

                <text
                textAnchor="middle"
                dy=".3em"
                className="text-xs font-medium fill-slate-700 dark:fill-slate-200 pointer-events-none" data-oid="uz5wsfv">
            ...
            <g
              ...
              className="cursor-pointer" data-oid=":6m4t07">

                <circle
                r={node.radius}
                className={`${getNodeColor(node)} border transition-all ${getSelectedClass(node)}`} data-oid="0w:can." />

                <text
                textAnchor="middle"
                dy=".3em"
                className="text-xs fill-slate-700 dark:fill-slate-200 pointer-events-none" data-oid="b.q_ge_">
        ...
        {/* Legend */}
        <div className="absolute bottom-3 left-3 bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm p-2 rounded-md text-xs border border-slate-200 dark:border-slate-700" data-oid="snubwk0">
          <div className="font-medium mb-1" data-oid="2axvu3v">Legend</div>
          <div className="flex items-center mb-1" data-oid="vdcl7yg">
            <div className="w-3 h-3 rounded-full bg-indigo-100 border border-indigo-300 mr-1" data-oid="koawb:l"></div>
            <span data-oid="6drh14t">Topic nodes</span>
          </div>
          <div className="flex items-center mb-1" data-oid="7rhyaof">
            <div className="w-3 h-3 rounded-full bg-teal-100 border border-teal-300 mr-1" data-oid="i.l_6k:"></div>
            <span data-oid="hcila6u">Subtopic nodes</span>
          </div>
          <div className="flex items-center" data-oid=".81b.re">
            <svg width="16" height="8" data-oid="bw82a6c">
              <line x1="0" y1="4" x2="16" y2="4" className="stroke-slate-300 dark:stroke-slate-600 stroke-[2]" data-oid="ka_lb8t" />
            </svg>
            <span className="ml-1" data-oid="donuv9n">Direct relations</span>
          </div>
          <div className="flex items-center" data-oid="_bheywd">
            <svg width="16" height="8" data-oid="8isy9.t">
              <line x1="0" y1="4" x2="16" y2="4" className="stroke-indigo-300 dark:stroke-indigo-700 stroke-[2] stroke-dasharray-[5,5]" data-oid="uh6xcgf" />
            </svg>
            <span className="ml-1" data-oid="b994s5o">Concept relations</span>
          </div>
        </div>
        
        {/* Instructions */}
        {!selectedTopic &&
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm p-3 rounded-md text-center border border-slate-200 dark:border-slate-700 shadow-lg" data-oid="qpn3t-8">
            <div className="font-medium mb-1" data-oid="cd0agyi">Interactive Concept Map</div>
            <p className="text-sm text-slate-600 dark:text-slate-400" data-oid="fdz1-44">
      ...
      {selectedTopic &&
      <div className="mt-4 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-200 dark:border-slate-700" data-oid="s_zuo3w">
          <div className="flex justify-between items-center mb-3" data-oid="cz96wgd">
            <h4 className="font-semibold" data-oid="dv0itin">{selectedTopic} Questions</h4>
            <button
            onClick={() => setSelectedTopic(null)}
            className="text-xs px-2 py-1 bg-white dark:bg-slate-700 rounded border border-slate-200 dark:border-slate-600" data-oid=".y8d_.y">

              Close
            </button>
          </div>
          
          <div className="space-y-2 max-h-[250px] overflow-y-auto" data-oid="-442b0z">
          ...
          <div
            ...
            'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50'}`
                ...
                <div data-oid="asg1y_p">
                  <div className="font-medium text-sm flex items-center" data-oid="yhq84ho">
                    <span className={`inline-block w-2 h-2 rounded-full mr-2 ${
                question.correct ?
                'bg-green-500 dark:bg-green-400' :
                'bg-red-500 dark:bg-red-400'}`
                } data-oid="4-3ss8d"></span>
                    {question.topic} - {question.subtopic}
                  </div>
                  <div className="text-xs text-slate-500 dark:text-slate-400" data-oid="rfw2-mo">
                    From {question.setType} • {question.setSubject}
                  </div>
                </div>
                <div className="text-right text-xs" data-oid="x:v8:kn">
          ...
          <div className="py-6 text-center text-slate-500 dark:text-slate-400" data-oid="634fsry">
                <p data-oid="q._blb2">No questions available for this topic.</p>
              </div>
          }
          </div>
        </div>
