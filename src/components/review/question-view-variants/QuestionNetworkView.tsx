...
export function QuestionNetworkView({ practiceSets, onSelectSet, selectedSetId }: QuestionViewProps) {
  ...
  return (
    <div className="border border-slate-200 dark:border-slate-700 rounded-xl p-4 shadow-sm">
      <h3 className="text-xl font-bold mb-6 text-center">12. Question Network View</h3>
      ...
      <div className="bg-slate-50 dark:bg-slate-800/50 p-3 rounded-lg mb-4 text-sm text-slate-600 dark:text-slate-300">
        <p>This network visualization shows relationships between questions. Questions are connected based on shared {
      ...
      <div className="aspect-video bg-white dark:bg-slate-800 rounded-lg shadow-sm overflow-hidden relative mb-6">
        <svg ref={svgRef} width="100%" height="100%" viewBox="0 0 100 100" className="font-sans">
          {/* Links */}
          {links.map((link, i) => {
          ...
          {nodes.map((node, i) => {
            ...
            return (
              <g key={`node-${i}`} onClick={() => handleNodeClick(node)} className="cursor-pointer">
                <circle
      ...
      {/* Legend */}
      <div className="bg-white dark:bg-slate-800 rounded-lg p-3 shadow-sm mb-4">
      ...
      <div className="text-sm text-slate-600 dark:text-slate-300">
        <p className="mb-2">Network Statistics:</p>
        <ul className="space-y-1 list-disc pl-5">
          <li>Questions: {nodes.length}</li>
          <li>Connections: {links.length}</li>
          <li>Topics: {new Set(nodes.map(n => n.question.topic)).size}</li>
          <li>Connected by: {
