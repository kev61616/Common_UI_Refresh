...
export function BookshelfView({ practiceSets, onSelectSet, selectedSetId }: QuestionViewProps) {
  ...
  return (
    <div className="border border-slate-200 dark:border-slate-700 rounded-xl p-4 shadow-sm" data-oid="zhldy-s">
      <h3 className="text-xl font-bold mb-6 text-center" data-oid="70.y0f7">28. Bookshelf View</h3>
      
      {/* Reading Room Controls */}
      <div className="mb-6 flex flex-wrap gap-4 justify-center" data-oid="5yhc7l1">
        <div className="bg-amber-50 dark:bg-slate-800 rounded-lg p-2 shadow border border-amber-200 dark:border-slate-700" data-oid="z.vd0tz">
          <label className="block text-sm text-amber-800 dark:text-amber-400 mb-1 font-serif" data-oid="rdi:q6m">Subject</label>
          <select
            ...
            onChange={(e) => setSelectedSubject(e.target.value)} data-oid="r.fujpu">

            <option value="all" data-oid="0vf5a.j">All Subjects</option>
            {subjects.map((subject) =>
            <option key={subject} value={subject} data-oid="vjzr0p-">{subject}</option>
            )}
          </select>
        </div>
        
        <div className="bg-amber-50 dark:bg-slate-800 rounded-lg p-2 shadow border border-amber-200 dark:border-slate-700" data-oid="hc8hg9i">
          <label className="block text-sm text-amber-800 dark:text-amber-400 mb-1 font-serif" data-oid="95fl7fd">Difficulty</label>
          <select
            className="bg-white dark:bg-slate-700 border border-amber-200 dark:border-slate-600 rounded px-2 py-1 text-sm font-serif"
            value={filterDifficulty}
            onChange={(e) => setFilterDifficulty(e.target.value)} data-oid="7p2dtjg">

            <option value="all" data-oid="omsskfy">All Difficulties</option>
            <option value="Easy" data-oid="yx-mz_c">Easy</option>
            <option value="Medium" data-oid="ht25-te">Medium</option>
            <option value="Hard" data-oid=":38y_mu">Hard</option>
          </select>
        </div>
      </div>
      
      {/* Library Display */}
      <div className="bg-gradient-to-b from-amber-100 to-amber-50 dark:from-slate-900 dark:to-slate-800 rounded-lg p-4 min-h-[600px] shadow-inner border border-amber-200 dark:border-slate-700" data-oid="8:whw4-">
        ...
        {/* Library header */}
        <div className="mb-8" data-oid="0:u_ke-">
          <div className="text-center" data-oid="f6:fl96">
            <h2 className="text-lg font-serif font-bold text-amber-900 dark:text-amber-300 border-b border-amber-200 dark:border-amber-800 pb-2 mb-2 mx-auto w-max px-10" data-oid="73x1ych">
              Library of Study
            </h2>
            <p className="text-xs italic text-amber-800 dark:text-amber-400 font-serif" data-oid="lmj2:0-">
        ...
        {/* The Bookshelves */}
        <div ref={shelfRef} className="space-y-10" data-oid="62:xz-4">
          {Object.entries(setsBySubject).length > 0 ?
          Object.entries(setsBySubject).map(([subject, sets], shelfIndex) =>
          <div key={subject} className="relative" data-oid="nq-cj0z">
                {/* Subject label */}
                <div className="absolute -top-6 left-4 bg-amber-700 dark:bg-amber-900 text-white px-3 py-1 text-xs font-medium rounded shadow-sm" data-oid="6d7ea3x">
                  {subject}
                </div>
                
                {/* Bookshelf with wooden texture */}
                <div className="relative bg-amber-800/90 dark:bg-amber-900/90 rounded-t-md min-h-[180px] p-4 shadow-md border border-amber-900/30 dark:border-amber-950/50" data-oid="8ek7.t0">
                  {/* Wood grain overlay */}
                  <div
                ...
                style={{
                  ...
                  {/* Books on the shelf */}
                  <div className="flex items-end space-x-1 relative z-10 h-full" data-oid="uim6egv">
                    {sets.map((set) => {
                      ...
                      onClick={() => {
                        ...
                        style={{
                          ...
                          boxShadow: `inset -2px 0 3px rgba(0,0,0,0.2)`
                            ...
                            <div className="absolute inset-0 flex items-center justify-center" data-oid="lz4faxa">
                              <div
                          ...
                          style={{
                            ...
                            {/* Book Bottom - Year */}
                            <div className="text-white/70 text-[8px] mb-1 mt-auto" data-oid="8:hmfpd">
                          ...
                          {isPulledOut &&
                      <div className="absolute -right-64 top-1/2 -translate-y-1/2 w-60 bg-white dark:bg-slate-800 rounded-md shadow-lg border border-amber-200 dark:border-slate-700 p-3 z-30" data-oid="bh2:oyr">
                              <div className="text-center mb-3" data-oid="ri1vsf2">
                                <h3 className="font-serif font-bold text-amber-900 dark:text-amber-400" data-oid="djbx0df">{set.type}</h3>
                                <div className="text-xs text-slate-500 dark:text-slate-400" data-oid="k4rjhxc">{set.subject}</div>
                              </div>
                              
                              <div className="space-y-2 text-sm" data-oid="lp5qyj0">
                                <div className="flex justify-between" data-oid="xfom1uh">
                                  <span className="text-slate-600 dark:text-slate-400" data-oid="afhd_lk">Questions:</span>
                                  <span className="font-medium" data-oid="ho4yjfc">{set.questions.length}</span>
                                </div>
                                <div className="flex justify-between" data-oid="163qlw1">
                                  <span className="text-slate-600 dark:text-slate-400" data-oid="lvd9vul">Accuracy:</span>
                                  <span className="font-medium" data-oid="_c3w3b5">{set.accuracy}%</span>
                                </div>
                                <div className="flex justify-between" data-oid="wcyae.j">
                                  <span className="text-slate-600 dark:text-slate-400" data-oid="a1zmxp0">Difficulty:</span>
                                  <span className="font-medium" data-oid="k1azv02">{set.difficulty}</span>
                                </div>
                                <div className="flex justify-between" data-oid=":w4mcy1">
                                  <span className="text-slate-600 dark:text-slate-400" data-oid="l0u1cq.">Completed:</span>
                                  <span className="font-medium" data-oid="7dk3i3a">{new Date(set.dateCompleted).toLocaleDateString()}</span>
                                </div>
                              </div>
                              
                              <div className="mt-4 pt-2 border-t border-slate-200 dark:border-slate-700 text-center" data-oid="b.y.num">
                                <button
                ...
                {/* Shelf support */}
                <div className="h-4 bg-amber-900 dark:bg-amber-950 rounded-b-md shadow-md relative" data-oid="ww86zy.">
          ...
          <div className="h-80 flex flex-col items-center justify-center text-amber-800 dark:text-amber-500" data-oid="oxxxx7.">
              <div className="text-center p-8 max-w-lg" data-oid="svlc-b2">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-12 w-12 mx-auto mb-4 opacity-50" fill="none" viewBox="0 0 24 24" stroke="currentColor" data-oid="6n2hpit">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" data-oid="w41.fr7" />
                </svg>
                <p className="font-serif text-lg font-medium mb-2" data-oid="tk-5w12">No Books on these Shelves</p>
                <p className="font-serif text-sm opacity-70" data-oid="v_r:.wy">
      ...
      {/* Description */}
      <div className="mt-4 bg-white dark:bg-slate-800 border border-amber-100 dark:border-slate-700 rounded-lg p-3 text-sm" data-oid="3manlq5">
        <p className="font-serif text-amber-900 dark:text-amber-400" data-oid="6wtvm3h">
