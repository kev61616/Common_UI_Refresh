...
// 3D crystal matrix with facets reflecting performance metrics across different dimensions
 ...
 */
export function CrystallineStructureView({ practiceSets, onSelectSet, selectedSetId }: QuestionViewProps) {
  ...
  return (
    <div className="border border-slate-200 dark:border-slate-700 rounded-xl p-4 shadow-sm" data-oid="kk__-ga">
      <h3 className="text-xl font-bold mb-6 text-center" data-oid="jwqo7_p">72. Crystalline Structure</h3>
      
      {/* Subject Filter */}
      <div className="mb-6" data-oid="ne6e3c2">
        <div className="flex space-x-2 overflow-x-auto pb-2" data-oid="4e3kaa7">
          <button
      ...
      {/* Matrix Grid */}
      <div className="overflow-x-auto mb-6" data-oid="r_zl4jo">
        <table className="w-full border-collapse" data-oid=":3zm02a">
          <thead data-oid="af:sq9a">
            <tr data-oid="5tlwuwr">
              <th className="p-3 text-left font-medium text-sm text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-800/50 border-b dark:border-slate-700" data-oid="poga2:g">
                Topic / Difficulty
              </th>
              ...
              <th
                ...
                'text-rose-600 dark:text-rose-400 bg-rose-50/50 dark:bg-rose-900/10'}`
                  ...
                  {difficulty}
                  <div className="text-xs font-normal mt-1 text-slate-500 dark:text-slate-400" data-oid="y4a73:1">
                    {difficultyTotals.find((d) => d.difficulty === difficulty)?.count || 0} questions
                  </div>
                </th>
              )}
              
              <th className="p-3 text-center font-medium text-sm text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-800/50 border-b dark:border-slate-700" data-oid="bl0:22w">
                Total
                <div className="text-xs font-normal mt-1" data-oid=":zbys7r">
          ...
          <tbody data-oid="egzi8..">
            {grid.map((row) =>
            <tr key={row.topic} data-oid="dzxbxf6">
                <td className="p-3 text-sm font-medium border-b dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50" data-oid="5eg4szl">
                  {row.topic}
                  <div className="text-xs font-normal text-slate-500 dark:text-slate-400" data-oid="q5p_esr">
              ...
              <td
                ...
                ''}`
                    ...
                    <div className={`h-24 p-3 cursor-pointer transition-all hover:shadow-md ${getCellColor(cell.accuracy, cell.count)}`} data-oid="2sjafke">
                      <div className="flex flex-col h-full justify-between" data-oid="a00_4ip">
                        <div className={`text-lg font-bold ${getTextColor(cell.accuracy, cell.count)}`} data-oid="x63zbm4">
                          {cell.count > 0 ? `${cell.accuracy}%` : '-'}
                        </div>
                        
                        <div data-oid="1vt1njy">
                          <div className="text-xs text-slate-500 dark:text-slate-400" data-oid="lx17me0">
                            {cell.count} question{cell.count !== 1 ? 's' : ''}
                          </div>
                          
                          {cell.count > 0 &&
                      <div className="w-full bg-slate-200 dark:bg-slate-700 h-1 mt-1 rounded-full overflow-hidden" data-oid="a_jd.tm">
                              <div
                          ...
                          style={{ width: `${cell.accuracy}%` }} data-oid="f_t:_1p">
                        </div>
                            </div>
                      }
                        </div>
                ...
                <td className="p-3 text-center border-b dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50" data-oid="rw6yix_">
                ...
                topicTotals.find((t) => t.topic === row.topic)?.count || 0)}`
                  ...
                  <div className="w-full bg-slate-200 dark:bg-slate-700 h-1 mt-1 rounded-full overflow-hidden" data-oid="cb-7k40">
                    <div
                    className={`h-full rounded-full ${
                    ...
                    style={{ width: `${topicTotals.find((t) => t.topic === row.topic)?.accuracy || 0}%` }} data-oid="eu20_-7">
                  </div>
                  </div>
                </td>
              </tr>
            ...
            {/* Grand Total Row */}
            <tr data-oid="-cmljs_">
              <td className="p-3 text-sm font-medium border-b dark:border-slate-700 bg-slate-100 dark:bg-slate-800" data-oid="33.g17z">
                All Topics
              </td>
              ...
              <td
                ...
                className="p-3 text-center border-b dark:border-slate-700 bg-slate-100 dark:bg-slate-800" data-oid="n3eueig">

                  <div className={`text-lg font-bold ${getTextColor(total.accuracy, total.count)}`} data-oid="2yf0a91">
                    {total.accuracy}%
                  </div>
                  
                  <div className="w-full bg-slate-200 dark:bg-slate-700 h-1 mt-1 rounded-full overflow-hidden" data-oid="qvrzcou">
                    <div
                    className={`h-full rounded-full ${
                    ...
                    style={{ width: `${total.accuracy}%` }} data-oid="z6-r8:_">
                  </div>
                  </div>
                </td>
              )}
              
              <td className="p-3 text-center border-b dark:border-slate-700 bg-indigo-50 dark:bg-indigo-900/20" data-oid="ku5x:bi">
                <div className={`text-lg font-bold ${getTextColor(grandTotal.accuracy, grandTotal.count)}`} data-oid="v5sxuvt">
                  {grandTotal.accuracy}%
                </div>
                
                <div className="w-full bg-slate-200 dark:bg-slate-700 h-1 mt-1 rounded-full overflow-hidden" data-oid="kqm32m9">
                  <div
                    className={`h-full rounded-full ${
                    ...
                    style={{ width: `${grandTotal.accuracy}%` }} data-oid="mcddv.z">
                  </div>
                </div>
              </td>
            </tr>
      ...
      {selectedCell &&
      <div className="mt-6 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-200 dark:border-slate-700" data-oid="v6ax1sk">
          <div className="flex justify-between items-center mb-3" data-oid="089ofya">
            <h4 className="font-semibold" data-oid="6.m_852">
              {selectedCell.topic} - {selectedCell.difficulty} Questions
            </h4>
            <button
            onClick={() => setSelectedCell(null)}
            className="text-xs px-2 py-1 bg-white dark:bg-slate-700 rounded border border-slate-200 dark:border-slate-600" data-oid="5b2xus1">

              Close
            </button>
          </div>
          
          <div className="space-y-2 max-h-[250px] overflow-y-auto" data-oid="o7tjqdi">
          ...
          <div
            ...
            'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50'}`
                ...
                <div data-oid="-awlxgn">
                  <div className="font-medium text-sm flex items-center" data-oid="k:mpcrt">
                    <span className={`inline-block w-2 h-2 rounded-full mr-2 ${
                question.correct ?
                'bg-green-500 dark:bg-green-400' :
                'bg-red-500 dark:bg-red-400'}`
                } data-oid="exr:0oh"></span>
                    {question.subtopic}
                  </div>
                  <div className="text-xs text-slate-500 dark:text-slate-400" data-oid="p4at9qi">
                    From {question.setType} • {question.setSubject}
                  </div>
                </div>
                <div className="text-right text-xs" data-oid=".g52fo.">
          ...
          <div className="py-6 text-center text-slate-500 dark:text-slate-400" data-oid="_i9hsqr">
                <p data-oid="1.dql.m">No questions available for this combination.</p>
              </div>
          }
          </div>
        </div>
