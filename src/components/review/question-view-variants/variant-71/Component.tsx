...
// Layered rock formations showing 'depth' of knowledge across topics with stratified layers
 ...
 */
export function GeologicalCrossSectionView({ practiceSets, onSelectSet, selectedSetId }: QuestionViewProps) {
  ...
  return (
    <div className="border border-slate-200 dark:border-slate-700 rounded-xl p-4 shadow-sm" data-oid="tl:487n">
      <h3 className="text-xl font-bold mb-6 text-center" data-oid="vir1-w0">71. Geological Cross Section</h3>
      
      {/* Subject Filter */}
      <div className="mb-6" data-oid="y...fl1">
        <div className="flex space-x-2 overflow-x-auto pb-2" data-oid="4updipt">
          <button
      ...
      {/* Matrix Grid */}
      <div className="overflow-x-auto mb-6" data-oid="dc-c0zd">
        <table className="w-full border-collapse" data-oid="4s9lv_a">
          <thead data-oid="c-79otl">
            <tr data-oid="apla847">
              <th className="p-3 text-left font-medium text-sm text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-800/50 border-b dark:border-slate-700" data-oid="6n4lkn9">
                Topic / Difficulty
              </th>
              ...
              <th
                ...
                'text-rose-600 dark:text-rose-400 bg-rose-50/50 dark:bg-rose-900/10'}`
                  ...
                  {difficulty}
                  <div className="text-xs font-normal mt-1 text-slate-500 dark:text-slate-400" data-oid="-zp1w9p">
                    {difficultyTotals.find((d) => d.difficulty === difficulty)?.count || 0} questions
                  </div>
                </th>
              )}
              
              <th className="p-3 text-center font-medium text-sm text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-800/50 border-b dark:border-slate-700" data-oid="9-tj8qh">
                Total
                <div className="text-xs font-normal mt-1" data-oid="tck3bq3">
          ...
          <tbody data-oid="7n:v4xz">
            {grid.map((row) =>
            <tr key={row.topic} data-oid="ip89k:g">
                <td className="p-3 text-sm font-medium border-b dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50" data-oid="h:cg1cu">
                  {row.topic}
                  <div className="text-xs font-normal text-slate-500 dark:text-slate-400" data-oid="y-penwr">
              ...
              <td
                ...
                ''}`
                    ...
                    <div className={`h-24 p-3 cursor-pointer transition-all hover:shadow-md ${getCellColor(cell.accuracy, cell.count)}`} data-oid="tkyw86_">
                      <div className="flex flex-col h-full justify-between" data-oid="wtk2jlt">
                        <div className={`text-lg font-bold ${getTextColor(cell.accuracy, cell.count)}`} data-oid="_m6own2">
                          {cell.count > 0 ? `${cell.accuracy}%` : '-'}
                        </div>
                        
                        <div data-oid="6a31-xr">
                          <div className="text-xs text-slate-500 dark:text-slate-400" data-oid="bgjuf7h">
                            {cell.count} question{cell.count !== 1 ? 's' : ''}
                          </div>
                          
                          {cell.count > 0 &&
                      <div className="w-full bg-slate-200 dark:bg-slate-700 h-1 mt-1 rounded-full overflow-hidden" data-oid="-9dp6fn">
                              <div
                          ...
                          style={{ width: `${cell.accuracy}%` }} data-oid="8.q8dfy">
                        </div>
                            </div>
                      }
                        </div>
                ...
                <td className="p-3 text-center border-b dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50" data-oid="5uijjbw">
                ...
                topicTotals.find((t) => t.topic === row.topic)?.count || 0)}`
                  ...
                  <div className="w-full bg-slate-200 dark:bg-slate-700 h-1 mt-1 rounded-full overflow-hidden" data-oid="sndceg1">
                    <div
                    className={`h-full rounded-full ${
                    ...
                    style={{ width: `${topicTotals.find((t) => t.topic === row.topic)?.accuracy || 0}%` }} data-oid="-f630ed">
                  </div>
                  </div>
                </td>
              </tr>
            ...
            {/* Grand Total Row */}
            <tr data-oid="r5gf9z8">
              <td className="p-3 text-sm font-medium border-b dark:border-slate-700 bg-slate-100 dark:bg-slate-800" data-oid="rg4m9qm">
                All Topics
              </td>
              ...
              <td
                ...
                className="p-3 text-center border-b dark:border-slate-700 bg-slate-100 dark:bg-slate-800" data-oid="ihjl4n9">

                  <div className={`text-lg font-bold ${getTextColor(total.accuracy, total.count)}`} data-oid="1p74woq">
                    {total.accuracy}%
                  </div>
                  
                  <div className="w-full bg-slate-200 dark:bg-slate-700 h-1 mt-1 rounded-full overflow-hidden" data-oid="cqkjdzf">
                    <div
                    className={`h-full rounded-full ${
                    ...
                    style={{ width: `${total.accuracy}%` }} data-oid="zz-zq45">
                  </div>
                  </div>
                </td>
              )}
              
              <td className="p-3 text-center border-b dark:border-slate-700 bg-indigo-50 dark:bg-indigo-900/20" data-oid="9.yd51y">
                <div className={`text-lg font-bold ${getTextColor(grandTotal.accuracy, grandTotal.count)}`} data-oid="b8pwij9">
                  {grandTotal.accuracy}%
                </div>
                
                <div className="w-full bg-slate-200 dark:bg-slate-700 h-1 mt-1 rounded-full overflow-hidden" data-oid="8sxi.uv">
                  <div
                    className={`h-full rounded-full ${
                    ...
                    style={{ width: `${grandTotal.accuracy}%` }} data-oid="c2w168_">
                  </div>
                </div>
              </td>
            </tr>
      ...
      {selectedCell &&
      <div className="mt-6 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-200 dark:border-slate-700" data-oid="v4u.lm:">
          <div className="flex justify-between items-center mb-3" data-oid="c7qbwqn">
            <h4 className="font-semibold" data-oid="-rcxmmh">
              {selectedCell.topic} - {selectedCell.difficulty} Questions
            </h4>
            <button
            onClick={() => setSelectedCell(null)}
            className="text-xs px-2 py-1 bg-white dark:bg-slate-700 rounded border border-slate-200 dark:border-slate-600" data-oid="0io9e2b">

              Close
            </button>
          </div>
          
          <div className="space-y-2 max-h-[250px] overflow-y-auto" data-oid="ywdz8z5">
          ...
          <div
            ...
            'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50'}`
                ...
                <div data-oid="oiob3x1">
                  <div className="font-medium text-sm flex items-center" data-oid="t3:k71g">
                    <span className={`inline-block w-2 h-2 rounded-full mr-2 ${
                question.correct ?
                'bg-green-500 dark:bg-green-400' :
                'bg-red-500 dark:bg-red-400'}`
                } data-oid="coxdb81"></span>
                    {question.subtopic}
                  </div>
                  <div className="text-xs text-slate-500 dark:text-slate-400" data-oid="4hrk:zj">
                    From {question.setType} • {question.setSubject}
                  </div>
                </div>
                <div className="text-right text-xs" data-oid="5entoqm">
          ...
          <div className="py-6 text-center text-slate-500 dark:text-slate-400" data-oid="lzk8wiy">
                <p data-oid="rdm5t8t">No questions available for this combination.</p>
              </div>
          }
          </div>
        </div>
