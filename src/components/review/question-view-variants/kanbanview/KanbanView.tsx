...
export function KanbanView({
  ...
  return (
    <div className="pb-8" data-oid="k:fhrnd">
      {/* Kanban board layout */}
      <div className="w-full h-full overflow-x-auto" data-oid="f.:0ytf">
        <div className="flex gap-6 pb-4 px-2 min-w-max" data-oid="0e7nt:k">
          {masteryLevels.map((level) =>
          <div
            key={level.id}
            className="w-[250px] flex flex-col h-full" data-oid="lh2xm-0">

              {/* Column header */}
              <div className={`p-3 rounded-t-lg ${level.color} border-t border-l border-r`} data-oid="1-yd9ku">
                <div className="flex items-center justify-between mb-1" data-oid="wui9-u1">
                  <h3 className={`font-semibold ${level.titleColor}`} data-oid="w75iwd8">{level.name}</h3>
                  <span className="inline-flex items-center justify-center w-7 h-7 text-xs bg-white dark:bg-slate-700 text-slate-700 dark:text-slate-300 font-semibold rounded-full" data-oid="gv5ye3.">
                    {questionsByLevel[level.id].length}
                  </span>
                </div>
                <div className="text-xs text-slate-600 dark:text-slate-400" data-oid=".x:ma5l">{level.description}</div>
              </div>
              
              {/* Column content */}
              <div className={`flex-grow p-3 overflow-y-auto rounded-b-lg border-b border-l border-r ${level.color} min-h-[400px]`} data-oid="bvafvu.">
                <div className="flex flex-col gap-3" data-oid="8wz1kzo">
                ...
                <div
                  ...
                  selectedSetId === question.setId ? 'ring-2 ring-indigo-500 dark:ring-indigo-400' : ''}`
                      ...
                      <div className="flex items-center gap-2 mb-2" data-oid="miymjl2">
                        <div className={`w-2 h-2 rounded-full ${getSubjectColor(question.setSubject)}`} data-oid="o.hrlif"></div>
                        <span className="text-xs font-medium text-slate-500 dark:text-slate-400" data-oid="un6kh4k">{question.setSubject}</span>
                      </div>
                      
                      <div className="font-medium text-sm mb-2 text-slate-800 dark:text-slate-200" data-oid="1j9yd8e">
                        {question.topic}
                      </div>
                      
                      <div className="text-xs text-slate-600 dark:text-slate-400" data-oid="vt1bukb">
                        {question.subtopic}
                      </div>
                      
                      <div className="flex items-center justify-between mt-3 pt-2 border-t border-slate-100 dark:border-slate-700" data-oid="-_0z057">
                    ...
                    'bg-rose-100 text-rose-700 dark:bg-rose-900/30 dark:text-rose-400'}`
                        ...
                        <span className="text-xs text-slate-500 dark:text-slate-400" data-oid="nzk8::h">
                ...
                <div className="flex flex-col items-center justify-center h-32 text-center" data-oid="ypvglzc">
                      <div className="text-slate-400 dark:text-slate-500 mb-2" data-oid="_qxi4a_">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor" data-oid="jfybjul">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" data-oid="skd4pg7" />
                        </svg>
                      </div>
                      <div className="text-xs text-slate-500 dark:text-slate-400" data-oid="f052ujn">No questions in this category</div>
                    </div>
                }
                </div>
              </div>
