...
export function AccordionCategoryView({ practiceSets, onSelectSet, selectedSetId }: QuestionViewProps) {
  ...
  return (
    <div className="border border-slate-200 dark:border-slate-700 rounded-xl p-4 shadow-sm">
      <h3 className="text-xl font-bold mb-6 text-center">15. Accordion Category View</h3>
      ...
      <div className="mb-6 flex flex-wrap gap-4 justify-center">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-lg p-2 shadow-sm">
          ...
          <select 
            ...
            value={groupBy}
            onChange={(e) => setGroupBy(e.target.value as 'topic' | 'difficulty' | 'subject')}
          >
        ...
        {/* Sort by control */}
        <div className="bg-white dark:bg-slate-800 rounded-lg p-2 shadow-sm">
          <label className="block text-sm text-slate-500 dark:text-slate-400 mb-1">Sort Questions By</label>
          <select 
            className="bg-slate-50 dark:bg-slate-700 text-slate-900 dark:text-white rounded-md px-2 py-1 text-sm border border-slate-200 dark:border-slate-600 w-full"
            value={sortBy}
            onChange={(e) => setSortBy(e.target.value as 'accuracy' | 'date' | 'time')}
          >
        ...
        {/* Expand/Collapse All */}
        <div className="bg-white dark:bg-slate-800 rounded-lg p-2 shadow-sm flex items-end">
          <div className="flex gap-2">
            <button 
              onClick={expandAll}
              className="px-3 py-1 text-xs font-medium bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-200 rounded hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors"
            >
              Expand All
            </button>
            <button 
          ...
          return (
            <div key={i} className="border border-slate-200 dark:border-slate-700 rounded-lg overflow-hidden">
              ...
              >
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <div className="flex items-center text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                      ...
                      <span>{stats.accuracy.toFixed(1)}% accuracy</span>
                      <span className="mx-2">•</span>
                      <span>{stats.avgTime.toFixed(1)}s avg time</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center">
                  <span className={`${getCategoryColor(category)} text-xs px-2 py-0.5 rounded mr-2`}>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex justify-between items-center text-xs text-slate-500 dark:text-slate-400">
                            <div>
                              <span className={`inline-block px-1.5 py-0.5 rounded ${getCategoryColor(question.difficulty)}`}>
                                {question.difficulty}
                              </span>
                            </div>
                            <div className="flex items-center">
      ...
      <div className="mt-4 text-sm text-slate-500 dark:text-slate-400 text-center">
        <p>Click on category headers to expand/collapse. Click on questions to view details.</p>
      </div>
    </div>
  )
}
