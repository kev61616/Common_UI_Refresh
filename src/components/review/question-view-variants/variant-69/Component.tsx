...
// Woven fabric-like visualization with different weave patterns indicating question performance
 ...
 */
export function TextilePatternGridView({ practiceSets, onSelectSet, selectedSetId }: QuestionViewProps) {
  ...
  return (
    <div className="border border-slate-200 dark:border-slate-700 rounded-xl p-4 shadow-sm" data-oid="pcd-brg">
      <h3 className="text-xl font-bold mb-6 text-center" data-oid="jgg01x1">69. Textile Pattern Grid</h3>
      
      {/* Subject Filter */}
      <div className="mb-6" data-oid="cyqspwq">
        <div className="flex space-x-2 overflow-x-auto pb-2" data-oid="_f7hdcv">
          <button
      ...
      {/* Matrix Grid */}
      <div className="overflow-x-auto mb-6" data-oid="4:m9nr1">
        <table className="w-full border-collapse" data-oid="rhzde:w">
          <thead data-oid="0fxj887">
            <tr data-oid="b747a-l">
              <th className="p-3 text-left font-medium text-sm text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-800/50 border-b dark:border-slate-700" data-oid="eb66f5q">
                Topic / Difficulty
              </th>
              ...
              <th
                ...
                'text-rose-600 dark:text-rose-400 bg-rose-50/50 dark:bg-rose-900/10'}`
                  ...
                  {difficulty}
                  <div className="text-xs font-normal mt-1 text-slate-500 dark:text-slate-400" data-oid="krzrwky">
                    {difficultyTotals.find((d) => d.difficulty === difficulty)?.count || 0} questions
                  </div>
                </th>
              )}
              
              <th className="p-3 text-center font-medium text-sm text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-800/50 border-b dark:border-slate-700" data-oid="m6av497">
                Total
                <div className="text-xs font-normal mt-1" data-oid="jjx5i3z">
          ...
          <tbody data-oid="hc_:dvj">
            {grid.map((row) =>
            <tr key={row.topic} data-oid="ewgn6jy">
                <td className="p-3 text-sm font-medium border-b dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50" data-oid="cs6j:4n">
                  {row.topic}
                  <div className="text-xs font-normal text-slate-500 dark:text-slate-400" data-oid="_ggx:7c">
              ...
              <td
                ...
                ''}`
                    ...
                    <div className={`h-24 p-3 cursor-pointer transition-all hover:shadow-md ${getCellColor(cell.accuracy, cell.count)}`} data-oid="yichfb2">
                      <div className="flex flex-col h-full justify-between" data-oid="eui3tdv">
                        <div className={`text-lg font-bold ${getTextColor(cell.accuracy, cell.count)}`} data-oid="p0rpzbn">
                          {cell.count > 0 ? `${cell.accuracy}%` : '-'}
                        </div>
                        
                        <div data-oid="ycf98y.">
                          <div className="text-xs text-slate-500 dark:text-slate-400" data-oid="pigzp7y">
                            {cell.count} question{cell.count !== 1 ? 's' : ''}
                          </div>
                          
                          {cell.count > 0 &&
                      <div className="w-full bg-slate-200 dark:bg-slate-700 h-1 mt-1 rounded-full overflow-hidden" data-oid="upwxsun">
                              <div
                          ...
                          style={{ width: `${cell.accuracy}%` }} data-oid="nkzi2z4">
                        </div>
                            </div>
                      }
                        </div>
                ...
                <td className="p-3 text-center border-b dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50" data-oid="l.8n4-.">
                ...
                topicTotals.find((t) => t.topic === row.topic)?.count || 0)}`
                  ...
                  <div className="w-full bg-slate-200 dark:bg-slate-700 h-1 mt-1 rounded-full overflow-hidden" data-oid="9jygvvf">
                    <div
                    className={`h-full rounded-full ${
                    ...
                    style={{ width: `${topicTotals.find((t) => t.topic === row.topic)?.accuracy || 0}%` }} data-oid="qh4zfoj">
                  </div>
                  </div>
                </td>
              </tr>
            ...
            {/* Grand Total Row */}
            <tr data-oid=".dm:3cg">
              <td className="p-3 text-sm font-medium border-b dark:border-slate-700 bg-slate-100 dark:bg-slate-800" data-oid="etzw72m">
                All Topics
              </td>
              ...
              <td
                ...
                className="p-3 text-center border-b dark:border-slate-700 bg-slate-100 dark:bg-slate-800" data-oid="yu8ns12">

                  <div className={`text-lg font-bold ${getTextColor(total.accuracy, total.count)}`} data-oid="0__.r-4">
                    {total.accuracy}%
                  </div>
                  
                  <div className="w-full bg-slate-200 dark:bg-slate-700 h-1 mt-1 rounded-full overflow-hidden" data-oid="a5eoh8l">
                    <div
                    className={`h-full rounded-full ${
                    ...
                    style={{ width: `${total.accuracy}%` }} data-oid="w2-s8uw">
                  </div>
                  </div>
                </td>
              )}
              
              <td className="p-3 text-center border-b dark:border-slate-700 bg-indigo-50 dark:bg-indigo-900/20" data-oid="s_:s7a_">
                <div className={`text-lg font-bold ${getTextColor(grandTotal.accuracy, grandTotal.count)}`} data-oid="sz9t8gq">
                  {grandTotal.accuracy}%
                </div>
                
                <div className="w-full bg-slate-200 dark:bg-slate-700 h-1 mt-1 rounded-full overflow-hidden" data-oid="xr5mbsj">
                  <div
                    className={`h-full rounded-full ${
                    ...
                    style={{ width: `${grandTotal.accuracy}%` }} data-oid="j3h7tot">
                  </div>
                </div>
              </td>
            </tr>
      ...
      {selectedCell &&
      <div className="mt-6 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-200 dark:border-slate-700" data-oid="3pp5uv-">
          <div className="flex justify-between items-center mb-3" data-oid="wl6een6">
            <h4 className="font-semibold" data-oid="gm671rw">
              {selectedCell.topic} - {selectedCell.difficulty} Questions
            </h4>
            <button
            onClick={() => setSelectedCell(null)}
            className="text-xs px-2 py-1 bg-white dark:bg-slate-700 rounded border border-slate-200 dark:border-slate-600" data-oid="kg2pr.-">

              Close
            </button>
          </div>
          
          <div className="space-y-2 max-h-[250px] overflow-y-auto" data-oid="zrjxtib">
          ...
          <div
            ...
            'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50'}`
                ...
                <div data-oid="xg9pc9b">
                  <div className="font-medium text-sm flex items-center" data-oid="394mnsa">
                    <span className={`inline-block w-2 h-2 rounded-full mr-2 ${
                question.correct ?
                'bg-green-500 dark:bg-green-400' :
                'bg-red-500 dark:bg-red-400'}`
                } data-oid="2zmxpyd"></span>
                    {question.subtopic}
                  </div>
                  <div className="text-xs text-slate-500 dark:text-slate-400" data-oid="3lc6lfx">
                    From {question.setType} • {question.setSubject}
                  </div>
                </div>
                <div className="text-right text-xs" data-oid="4dcil3l">
          ...
          <div className="py-6 text-center text-slate-500 dark:text-slate-400" data-oid="wfw7zxg">
                <p data-oid="l6b71qz">No questions available for this combination.</p>
              </div>
          }
          </div>
        </div>
