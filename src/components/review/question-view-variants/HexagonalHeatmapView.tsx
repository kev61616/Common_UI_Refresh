...
export function HexagonalHeatmapView({ practiceSets, onSelectSet, selectedSetId }: QuestionViewProps) {
    ...
    return hexData.map(hex => {
      ...
      return (
        <g 
          ...
          onClick={() => setSelectedHex(hex.id === selectedHex ? null : hex.id)}
        >
          {/* Shadow */}
          <path 
            d={hexPath} 
          ...
          <path 
            d={hexPath} 
            className={`${hex.color} ${isSelected ? 'stroke-indigo-500 dark:stroke-indigo-400 stroke-[3px]' : 'stroke-white dark:stroke-slate-700 stroke-2'}`}
          />
          ...
          <text 
            ...
            className={`text-sm font-medium ${hex.color.includes('text-') ? hex.color.split(' ').find(c => c.startsWith('text-')) : 'text-white'}`}
          >
          ...
          <text 
            ...
            textAnchor="middle" 
            dominantBaseline="middle"
            className={`text-xs ${hex.color.includes('text-') ? hex.color.split(' ').find(c => c.startsWith('text-')) : 'text-white'}`}
          >
  ...
  return (
    <div className="border border-slate-200 dark:border-slate-700 rounded-xl p-4 shadow-sm">
      <h3 className="text-xl font-bold mb-6 text-center">40. Hexagonal Heatmap View</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-5xl mx-auto">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-lg shadow-sm p-4 flex items-center justify-center">
          <div className="relative w-full h-[500px] overflow-hidden">
            <svg width="600" height="500" viewBox="0 0 600 500" className="max-w-full">
              {generateHexagons()}
            </svg>
          </div>
        </div>
      ...
      <div className="flex flex-wrap justify-center items-center mt-8 gap-4 text-xs">
        <div className="font-medium text-slate-600 dark:text-slate-300">Hexagon Color: Accuracy</div>
        ...
        <div className="font-medium text-slate-600 dark:text-slate-300 ml-4">Hexagon Size: Question Count</div>
      </div>
    </div>
  )
}
