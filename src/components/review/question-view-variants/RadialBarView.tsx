...
export function RadialBarView({ practiceSets, onSelectSet, selectedSetId }: QuestionViewProps) {
  ...
  const generateRadialBars = () => {
    ...
    return barData.map((bar, index) => {
      ...
      return (
        <g key={bar.id} onClick={() => setSelectedBar(bar.id === selectedBar ? null : bar.id)}>
          {/* Bar */}
          <line
            x1={position.innerX}
            y1={position.innerY}
          ...
          <text
            ...
            transform={`rotate(${labelAngle}, ${position.labelX}, ${position.labelY})`}
          >
            {bar.label}
          </text>
        </g>
  ...
  return (
    <div className="border border-slate-200 dark:border-slate-700 rounded-xl p-4 shadow-sm">
      <h3 className="text-xl font-bold mb-6 text-center">39. Radial Bar View</h3>
      ...
      <div className="flex flex-wrap justify-center items-center mt-8 gap-4 text-xs">
        <div className="font-medium text-slate-600 dark:text-slate-300">Bar Color: {viewBy === 'topic' ? 'Accuracy' : 'Difficulty'}</div>
        {viewBy === 'topic' ? (
          <>
        ...
        <div className="font-medium text-slate-600 dark:text-slate-300 ml-4">Bar Length: Question Count</div>
      </div>
    </div>
  )
}
