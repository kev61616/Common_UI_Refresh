...
// Chemistry-inspired grid with elemental styling for topics and questions arranged by properties
 ...
 */
export function PeriodicTableOfKnowledgeView({ practiceSets, onSelectSet, selectedSetId }: QuestionViewProps) {
  ...
  return (
    <div className="border border-slate-200 dark:border-slate-700 rounded-xl p-4 shadow-sm" data-oid="39rjv65">
      <h3 className="text-xl font-bold mb-6 text-center" data-oid="2t8tm8n">63. Periodic Table of Knowledge</h3>
      
      {/* Subject Filter */}
      <div className="mb-6" data-oid="-.ycpcw">
        <div className="flex space-x-2 overflow-x-auto pb-2" data-oid="es8.wdj">
          <button
      ...
      {/* Matrix Grid */}
      <div className="overflow-x-auto mb-6" data-oid="-2kxjf.">
        <table className="w-full border-collapse" data-oid="q32ht8j">
          <thead data-oid="67ua.8w">
            <tr data-oid="plyl2q:">
              <th className="p-3 text-left font-medium text-sm text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-800/50 border-b dark:border-slate-700" data-oid="h2oon-l">
                Topic / Difficulty
              </th>
              ...
              <th
                ...
                'text-rose-600 dark:text-rose-400 bg-rose-50/50 dark:bg-rose-900/10'}`
                  ...
                  {difficulty}
                  <div className="text-xs font-normal mt-1 text-slate-500 dark:text-slate-400" data-oid=".0m1o8o">
                    {difficultyTotals.find((d) => d.difficulty === difficulty)?.count || 0} questions
                  </div>
                </th>
              )}
              
              <th className="p-3 text-center font-medium text-sm text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-800/50 border-b dark:border-slate-700" data-oid="aci7h11">
                Total
                <div className="text-xs font-normal mt-1" data-oid="3ndnero">
          ...
          <tbody data-oid="bs3yord">
            {grid.map((row) =>
            <tr key={row.topic} data-oid="t.34yy_">
                <td className="p-3 text-sm font-medium border-b dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50" data-oid="g-tabqf">
                  {row.topic}
                  <div className="text-xs font-normal text-slate-500 dark:text-slate-400" data-oid="ryz9gor">
              ...
              <td
                ...
                ''}`
                    ...
                    <div className={`h-24 p-3 cursor-pointer transition-all hover:shadow-md ${getCellColor(cell.accuracy, cell.count)}`} data-oid="zu86w7u">
                      <div className="flex flex-col h-full justify-between" data-oid="pvvkf0b">
                        <div className={`text-lg font-bold ${getTextColor(cell.accuracy, cell.count)}`} data-oid="g4-xvqy">
                          {cell.count > 0 ? `${cell.accuracy}%` : '-'}
                        </div>
                        
                        <div data-oid="uwmpe30">
                          <div className="text-xs text-slate-500 dark:text-slate-400" data-oid="_lb91e7">
                            {cell.count} question{cell.count !== 1 ? 's' : ''}
                          </div>
                          
                          {cell.count > 0 &&
                      <div className="w-full bg-slate-200 dark:bg-slate-700 h-1 mt-1 rounded-full overflow-hidden" data-oid="8fuc2d5">
                              <div
                          ...
                          style={{ width: `${cell.accuracy}%` }} data-oid="sgyw_0o">
                        </div>
                            </div>
                      }
                        </div>
                ...
                <td className="p-3 text-center border-b dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50" data-oid="618:4xc">
                ...
                topicTotals.find((t) => t.topic === row.topic)?.count || 0)}`
                  ...
                  <div className="w-full bg-slate-200 dark:bg-slate-700 h-1 mt-1 rounded-full overflow-hidden" data-oid="oj:8z.r">
                    <div
                    className={`h-full rounded-full ${
                    ...
                    style={{ width: `${topicTotals.find((t) => t.topic === row.topic)?.accuracy || 0}%` }} data-oid="8q..gak">
                  </div>
                  </div>
                </td>
              </tr>
            ...
            {/* Grand Total Row */}
            <tr data-oid="z6hal96">
              <td className="p-3 text-sm font-medium border-b dark:border-slate-700 bg-slate-100 dark:bg-slate-800" data-oid="ef.vr14">
                All Topics
              </td>
              ...
              <td
                ...
                className="p-3 text-center border-b dark:border-slate-700 bg-slate-100 dark:bg-slate-800" data-oid="lx3u342">

                  <div className={`text-lg font-bold ${getTextColor(total.accuracy, total.count)}`} data-oid="5ts72q:">
                    {total.accuracy}%
                  </div>
                  
                  <div className="w-full bg-slate-200 dark:bg-slate-700 h-1 mt-1 rounded-full overflow-hidden" data-oid="_0ggq-0">
                    <div
                    className={`h-full rounded-full ${
                    ...
                    style={{ width: `${total.accuracy}%` }} data-oid=".zi9s1.">
                  </div>
                  </div>
                </td>
              )}
              
              <td className="p-3 text-center border-b dark:border-slate-700 bg-indigo-50 dark:bg-indigo-900/20" data-oid="x.9o0f0">
                <div className={`text-lg font-bold ${getTextColor(grandTotal.accuracy, grandTotal.count)}`} data-oid="10g6c8b">
                  {grandTotal.accuracy}%
                </div>
                
                <div className="w-full bg-slate-200 dark:bg-slate-700 h-1 mt-1 rounded-full overflow-hidden" data-oid="r9-ltp8">
                  <div
                    className={`h-full rounded-full ${
                    ...
                    style={{ width: `${grandTotal.accuracy}%` }} data-oid="-bxj0ly">
                  </div>
                </div>
              </td>
            </tr>
      ...
      {selectedCell &&
      <div className="mt-6 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-200 dark:border-slate-700" data-oid="iiotu9d">
          <div className="flex justify-between items-center mb-3" data-oid="lu-q.bj">
            <h4 className="font-semibold" data-oid="8s0liot">
              {selectedCell.topic} - {selectedCell.difficulty} Questions
            </h4>
            <button
            onClick={() => setSelectedCell(null)}
            className="text-xs px-2 py-1 bg-white dark:bg-slate-700 rounded border border-slate-200 dark:border-slate-600" data-oid="6_w8m6u">

              Close
            </button>
          </div>
          
          <div className="space-y-2 max-h-[250px] overflow-y-auto" data-oid="_25hd7s">
          ...
          <div
            ...
            'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50'}`
                ...
                <div data-oid="_5j4tt6">
                  <div className="font-medium text-sm flex items-center" data-oid="110f2nr">
                    <span className={`inline-block w-2 h-2 rounded-full mr-2 ${
                question.correct ?
                'bg-green-500 dark:bg-green-400' :
                'bg-red-500 dark:bg-red-400'}`
                } data-oid="_ngujl_"></span>
                    {question.subtopic}
                  </div>
                  <div className="text-xs text-slate-500 dark:text-slate-400" data-oid="yk4bbyl">
                    From {question.setType} • {question.setSubject}
                  </div>
                </div>
                <div className="text-right text-xs" data-oid="kz78mci">
          ...
          <div className="py-6 text-center text-slate-500 dark:text-slate-400" data-oid="n0k_ph:">
                <p data-oid="xp._1r:">No questions available for this combination.</p>
              </div>
          }
          </div>
        </div>
