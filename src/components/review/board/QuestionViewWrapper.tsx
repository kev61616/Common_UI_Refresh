...
export function QuestionViewWrapper(props: ExtendedQuestionViewProps) {
  ...
  if (!isClient) {
    return <div className="min-h-[600px] bg-slate-50 dark:bg-slate-900 rounded-lg p-8" data-oid="zy-:x0c">
      <div className="animate-pulse flex flex-col space-y-4" data-oid="56y11mg">
        <div className="h-8 bg-slate-200 dark:bg-slate-700 rounded w-1/3 mx-auto" data-oid="0q5__7t"></div>
        <div className="h-4 bg-slate-200 dark:bg-slate-700 rounded w-2/3 mx-auto" data-oid=":cfu9hk"></div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-8" data-oid="el_lgp2">
          {[1, 2, 3].map((i) =>
          <div key={i} className="bg-slate-200 dark:bg-slate-700 h-48 rounded-lg" data-oid="nq1irme"></div>
          )}
        </div>
      </div>
    </div>;
  ...
  return <EnhancedCollapsibleBoardView {...props} data-oid="tr4jypv" />;
}