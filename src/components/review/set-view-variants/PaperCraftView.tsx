...
export function PaperCraftView({ practiceSets, onSelectSet, selectedSetId }: SetViewProps) {
  return (
    <div className="border border-slate-200 dark:border-slate-700 rounded-xl p-4 shadow-sm">
      <h3 className="text-xl font-bold mb-6 text-center">24. Paper Craft View</h3>
      
      <div className="relative bg-[#f5f3ee] dark:bg-slate-900 p-8 rounded-lg overflow-hidden">
        ...
        <div className="absolute left-0 top-0 bottom-0 w-6 bg-gradient-to-r from-amber-800/10 to-transparent dark:from-amber-950/20 flex flex-col justify-center items-center gap-4">
          {[...Array(10)].map((_, i) => (
            <div key={i} className="w-2 h-2 rounded-full bg-amber-900/20 dark:bg-amber-700/30"></div>
          ))}
        </div>
          ...
          {practiceSets.map((set, index) => (
            ...
            >
              ...
              <div className={`relative bg-white dark:bg-slate-800 rounded-md overflow-hidden 
                ...
                {/* Folded corner */}
                <div className="absolute top-0 right-0 w-10 h-10 bg-gradient-to-br from-slate-100 via-slate-200 to-slate-300 dark:from-slate-700 dark:via-slate-600 dark:to-slate-500 shadow-[-2px_2px_3px_rgba(0,0,0,0.05)]" style={{ clipPath: 'polygon(100% 0, 0 0, 100% 100%)' }}></div>
                <div className="absolute top-0 right-0 w-8 h-8 bg-white dark:bg-slate-800" style={{ clipPath: 'polygon(100% 0, 0 0, 100% 100%)' }}></div>
                ...
                <div className="p-5">
                  ...
                  <div className="flex justify-between items-start mb-5">
                    <div>
                      <div className="text-sm font-medium text-slate-500 dark:text-slate-400">{set.type}</div>
                      <h4 className="text-xl font-bold mt-1 text-slate-800 dark:text-white">{set.subject}</h4>
                    </div>
                    ...
                    >
                      ...
                      <svg className="absolute inset-0 w-full h-full" viewBox="0 0 50 50">
                        ...
                        <path 
                          ...
                          strokeWidth="1.5"
                          strokeLinecap="round"
                          strokeDasharray={`${(set.accuracy / 100) * 125} 125`}
                        />
                      </svg>
                      <span className="text-lg font-bold">{set.accuracy}<span className="text-xs">%</span></span>
                    </div>
                  </div>
                  
                  {/* Paper note lines */}
                  {[...Array(3)].map((_, i) => (
                    <div key={i} className="flex items-center mb-3 gap-2">
                  ...
                  <div className="mt-6 flex flex-wrap gap-2">
                    ...
                    <div className="px-2.5 py-1 rounded-full border border-slate-200 bg-slate-50 text-slate-600 text-xs dark:border-slate-700 dark:bg-slate-800 dark:text-slate-400">
                ...
                {selectedSetId === set.id && (
                  <div className="absolute -top-2 -left-2 transform -rotate-12 w-8 h-14 pointer-events-none">
              ...
              <div className="absolute inset-x-0 h-2 bottom-[-4px] bg-gradient-to-b from-black/5 to-transparent rounded-b-lg"></div>
            </div>
          ))}
        </div>
      </div>
