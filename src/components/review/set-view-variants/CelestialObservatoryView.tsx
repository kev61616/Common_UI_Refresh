...
export function CelestialObservatoryView({ practiceSets, onSelectSet, selectedSetId }: SetViewProps) {
  ...
  if (!practiceSets || practiceSets.length === 0) {
    return (
      <div className="border border-slate-200 dark:border-slate-700 rounded-xl p-4 shadow-sm">
        <h3 className="text-xl font-bold mb-6 text-center">56. Celestial Observatory View</h3>
        <div className="p-8 text-center text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-800/50 rounded-xl">
          <p className="mb-4">No celestial bodies detected in the observatory.</p>
          <p>Complete some sets to begin your astronomical observations.</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="border border-slate-200 dark:border-slate-700 rounded-xl p-4 shadow-sm">
      ...
      {/* Telescope View */}
      {viewMode === 'telescope' && (
        <div className="relative bg-slate-950 rounded-xl overflow-hidden min-h-[600px]">
          ...
          {/* Telescope frame */}
          <div className="absolute inset-0 border-[80px] border-black rounded-[50%] pointer-events-none"></div>
          <div className="absolute inset-16 border border-slate-700/30 rounded-full"></div>
          <div className="absolute inset-20 border border-slate-700/20 rounded-full"></div>
          ...
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            <div className="h-full w-[1px] bg-slate-700/20"></div>
          </div>
          
          {/* Celestial bodies */}
          <div className="relative h-full flex items-center justify-center">
            <div className="w-full max-w-3xl h-[440px] relative">
                ...
                return (
                  <React.Fragment key={set.id}>
                    ...
                    >
                      ...
                      {bodyType === 'comet' && (
                        <div className="absolute right-0 h-[1px] w-12 bg-gradient-to-l from-cyan-300 to-transparent"></div>
                      )}
                    </div>
                    
                    {/* Detailed view when selected */}
                    {isSelected && (
                      <div className="absolute z-50 bottom-4 left-1/2 -translate-x-1/2 bg-black/80 backdrop-blur-sm border border-slate-700 rounded-lg p-4 w-80">
                        <div className="flex gap-4">
                          <div className={`w-12 h-12 ${colorClass} ${nebulaClasses} ${glowEffect}`}></div>
                          <div>
                            <h4 className="text-white font-bold">{set.subject} - {set.type}</h4>
                            <div className="text-xs text-slate-400">{bodyType.charAt(0).toUpperCase() + bodyType.slice(1)}</div>
                          </div>
                        </div>
                        
                        <div className="mt-3 grid grid-cols-2 gap-2 text-xs text-slate-300">
                          <div>
                            <div className="text-slate-500">Magnitude</div>
                            <div>{magnitude.toFixed(1)}</div>
                          </div>
                          <div>
          ...
          <div className="absolute top-4 left-4 bg-black/60 backdrop-blur-sm border border-slate-800 rounded px-3 py-1.5 text-green-400 font-mono text-xs">
            Zoom: 20x
          </div>
          <div className="absolute top-4 right-4 bg-black/60 backdrop-blur-sm border border-slate-800 rounded px-3 py-1.5 text-green-400 font-mono text-xs">
            RA: 16h 42m • Dec: -12° 56'
          </div>
          <div className="absolute bottom-4 right-4 bg-black/60 backdrop-blur-sm border border-slate-800 rounded px-3 py-1.5 text-green-400 font-mono text-xs">
      ...
      {/* Star Chart View */}
      {viewMode === 'chart' && (
        <div className="relative bg-slate-900 rounded-xl overflow-hidden min-h-[600px] p-6">
          {/* Chart title */}
          <div className="absolute top-4 left-1/2 -translate-x-1/2 text-white font-serif italic">
            <h3 className="text-lg text-center">Celestial Chart of Learning Progress</h3>
            <div className="text-sm text-center text-slate-400">Northern Hemisphere • Academic Year 2025</div>
          </div>
          ...
          <div className="relative h-full pt-12">
            ...
            {['Math', 'Reading', 'Writing'].map(subject => {
              ...
              return (
                <svg key={`constellation-${subject}`} className="absolute inset-0 w-full h-full pointer-events-none">
                  {constellationLines}
                </svg>
              )
            })}
                ...
                >
                  ...
                  <div 
                    className={`absolute top-full left-1/2 -translate-x-1/2 mt-1 text-[8px] font-mono pointer-events-none
                      ${selectedSetId === set.id ? 'text-white' : 'text-slate-400'}`}
                  >
                  ...
                  {selectedSetId === set.id && (
                    <div className="absolute top-8 left-1/2 -translate-x-1/2 mt-2 bg-slate-950/80 backdrop-blur-sm border border-slate-700 rounded p-2 w-48 pointer-events-none z-50">
                      <div className="text-white text-xs font-medium">{set.subject} {set.type}</div>
                      <div className="text-slate-400 text-[10px]">{classification} ({bodyType})</div>
                      <div className="mt-1 text-[10px] text-slate-300 grid grid-cols-2 gap-x-1">
                        <div>RA: {coordinates.rightAscension}</div>
                        <div>Dec: {coordinates.declination}</div>
                      </div>
                      <div className="mt-1 text-[10px] text-slate-300 grid grid-cols-2 gap-x-1">
                        <div>Magnitude: {(10 - set.accuracy / 10).toFixed(1)}</div>
            ...
            {['Math', 'Reading', 'Writing'].map(subject => {
              ...
              return (
                ...
                >
                  <div className="text-xs font-serif italic text-slate-400/70">
                    {constellationName}
                  </div>
                </div>
              )
          ...
          {/* Chart decorations */}
          <div className="absolute bottom-6 left-1/2 -translate-x-1/2 text-center text-[10px] text-slate-500 font-serif italic">
            <div>Observed from Syntaxia Observatory • Coordinates 40.7° N, 73.9° W</div>
            <div className="mt-1">Chart prepared by Astronomical Society of Educational Progress</div>
          </div>
          
          {/* Compass directions */}
          <div className="absolute top-8 left-8 text-xs text-slate-500">W</div>
          <div className="absolute top-8 right-8 text-xs text-slate-500">E</div>
          <div className="absolute bottom-8 left-1/2 -translate-x-1/2 text-xs text-slate-500">S</div>
          <div className="absolute top-8 left-1/2 -translate-x-1/2 text-xs text-slate-500">N</div>
          
          {/* Legend */}
          <div className="absolute bottom-8 right-8 bg-slate-900/80 backdrop-blur-sm border border-slate-700 rounded-lg p-3">
            <h4 className="text-xs text-slate-300 mb-2">Celestial Bodies</h4>
            <div className="space-y-1.5 text-[10px]">
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 bg-yellow-300 star-shape"></div>
                <span className="text-slate-400">Star (90%+)</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 bg-emerald-400 rounded-full"></div>
                <span className="text-slate-400">Planet (80%+)</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 bg-gray-300 rounded-full"></div>
                <span className="text-slate-400">Moon (70%+)</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 bg-indigo-400/50 rounded-full"></div>
                <span className="text-slate-400">Nebula (60%+)</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 bg-cyan-300 rounded-full"></div>
                <span className="text-slate-400">Comet (50%+)</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 bg-violet-600 rounded-full"></div>
                <span className="text-slate-400">Galaxy (Below 50%)</span>
              </div>
            </div>
          </div>
        </div>
      ...
      {viewMode === 'spectral' && (
        <div className="relative bg-slate-900 rounded-xl overflow-hidden min-h-[600px]">
                ...
                return (
                  ...
                  >
                    <div className="bg-slate-800 px-3 py-2 border-b border-slate-700 flex justify-between items-center">
                      <div>
                        <span className="text-white text-sm font-medium">{set.subject} {set.type}</span>
                        <span className="ml-2 text-xs text-slate-400">{bodyType}</span>
                      </div>
                      <div className="text-xs font-mono bg-slate-900 px-2 py-0.5 rounded border border-slate-700 text-indigo-300">
                        Class {spectralClass}
                      </div>
                    </div>
                    ...
                    {/* Spectral graph */}
                    <div className="p-4">
                      <div className="relative h-8 w-full bg-gradient-to-r overflow-hidden rounded-sm mb-2">
                        {/* Colored spectrum */}
                        <div className={`absolute inset-0 h-full w-full bg-gradient-to-r ${getSpectralColor()}`}></div>
                      ...
                      <div className="grid grid-cols-3 gap-2 mt-4 text-xs text-slate-300">
                        <div>
                          <div className="text-slate-500">Spectral Class</div>
                          <div>{spectralClass}-type</div>
                        </div>
                        <div>
                        ...
                        <div>
                          ...
                          <div>{Math.round(5000 + (set.accuracy * 30))}K</div>
                        </div>
                        <div>
                          <div className="text-slate-500">Luminosity</div>
                          <div>{set.accuracy}%</div>
                        </div>
                        <div>
                          <div className="text-slate-500">Observed</div>
                      ...
                      <div className="mt-4 pt-3 border-t border-slate-700">
                        <div className="text-xs text-slate-500 mb-1">Key Elements Detected:</div>
          ...
          <div className="absolute bottom-4 left-4 bg-black/60 backdrop-blur-sm border border-slate-800 rounded-lg p-3 text-xs">
            <h4 className="font-bold text-white mb-2">Spectral Classification</h4>
            <div className="grid grid-cols-2 gap-x-6 gap-y-1">
              <div className="flex items-center gap-1.5">
                <div className="w-2 h-2 rounded-full bg-blue-400"></div>
                <span className="text-blue-300">O - Highest Accuracy</span>
              </div>
              <div className="flex items-center gap-1.5">
                <div className="w-2 h-2 rounded-full bg-red-500"></div>
                <span className="text-red-300">M - Needs Review</span>
              </div>
              <div className="flex items-center gap-1.5">
                <div className="w-2 h-2 rounded-full bg-yellow-300"></div>
                <span className="text-yellow-200">G - Good Progress</span>
              </div>
              <div className="flex items-center gap-1.5">
                <div className="w-2 h-2 rounded-full bg-red-300"></div>
                <span className="text-red-200">K - Work Required</span>
              </div>
            </div>
          </div>
        </div>
