...
export function Component({ practiceSets, selectedSetId, onSelectSet }: SetViewProps) {
  ...
  return (
    <div className="p-6 bg-gray-50 dark:bg-gray-900 rounded-lg shadow-lg overflow-hidden">
      ...
      <div className="bg-black/90 dark:bg-black rounded-xl p-8 relative min-h-[500px] overflow-hidden">
        {/* Grid lines effect */}
        <div className="absolute inset-0 grid grid-cols-12 grid-rows-12">
          {Array.from({ length: 12 }).map((_, i) => (
            <div key={`h-${i}`} className="w-full h-px bg-blue-500/10 dark:bg-blue-400/20" style={{ gridRow: i + 1 }} />
          ))}
          {Array.from({ length: 12 }).map((_, i) => (
            <div key={`v-${i}`} className="h-full w-px bg-blue-500/10 dark:bg-blue-400/20" style={{ gridColumn: i + 1 }} />
          ))}
        </div>
        
        {/* Holographic projector "lights" at corners */}
        <div className="absolute top-2 left-2 w-3 h-3 bg-blue-400 rounded-full opacity-70 dark:bg-blue-500 animate-pulse" />
        <div className="absolute top-2 right-2 w-3 h-3 bg-blue-400 rounded-full opacity-70 dark:bg-blue-500 animate-pulse" style={{ animationDelay: '0.5s' }} />
        <div className="absolute bottom-2 left-2 w-3 h-3 bg-blue-400 rounded-full opacity-70 dark:bg-blue-500 animate-pulse" style={{ animationDelay: '1s' }} />
        <div className="absolute bottom-2 right-2 w-3 h-3 bg-blue-400 rounded-full opacity-70 dark:bg-blue-500 animate-pulse" style={{ animationDelay: '1.5s' }} />
        
        {/* Main content */}
        <div>
          {Object.entries(setsByDifficulty).map(([difficulty, sets], diffIndex) => (
            <div key={difficulty} className="mb-8 last:mb-0">
              <h3 className="text-lg font-medium text-white/90 mb-4 border-b border-white/10 pb-2">
                {difficulty} Projections
              </h3>
              
              <div className={`grid gap-5 ${sets.length > 0 ? 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3' : ''}`}>
                {sets.length > 0 ? sets.map(set => {
                  ...
                  return (
                    ...
                    >
                        ...
                        `}>
                          ...
                          <div className="grid grid-cols-2 gap-2 mt-auto">
                            <div className="flex flex-col">
                              ...
                              <span className="text-white">{set.questions.length}</span>
                            </div>
                            <div className="flex flex-col">
                              <span className="text-xs text-white/50">Time</span>
                              <span className="text-white">{Math.floor(set.timeUsed / 60)}m</span>
                            </div>
                          </div>
                        </div>
                      ...
                      {/* Projection base */}
                      <div className="h-1.5 w-full bg-gradient-to-r from-transparent via-blue-400/30 dark:via-blue-500/40 to-transparent rounded-full mt-1 mx-auto" style={{ width: '80%' }}></div>
                      ...
                      {(isHovered || isSelected) && (
                        ...
                        `}>
                          <div className="text-xs font-medium mb-2">Performance Analysis:</div>
                          <div className="grid grid-cols-2 gap-3 text-sm">
                            ...
                            <div>
                              <div className="text-white/60 text-xs">Conceptual Errors</div>
                              <div className="text-white font-medium">{set.mistakeTypes.conceptual}</div>
                            </div>
                            <div>
                              <div className="text-white/60 text-xs">Careless Errors</div>
                              <div className="text-white font-medium">{set.mistakeTypes.careless}</div>
                            </div>
                            <div>
                              <div className="text-white/60 text-xs">Early Accuracy</div>
                              <div className="text-white font-medium">{set.sessionFatigue.earlyAccuracy}%</div>
                            </div>
                            <div>
                              <div className="text-white/60 text-xs">Late Accuracy</div>
                              <div className="text-white font-medium">{set.sessionFatigue.lateAccuracy}%</div>
                            </div>
                          </div>
                        </div>
                      )}
                  ...
                  <div className="col-span-full text-center p-6 rounded-lg border border-white/10 text-white/50 italic">
