...
export const ZodiacConstellationView: React.FC<SetViewProps> = ({ 
  ...
  if (!sets || sets.length === 0) {
    return (
      <div className="min-h-[500px] flex items-center justify-center bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-lg">
        <p className="text-slate-500 dark:text-slate-400">No constellations available</p>
      </div>
    )
  }
    ...
    return (
      <div 
        ...
        className={`relative border rounded-lg overflow-hidden transition-all duration-300 cursor-pointer
                  ${styling.bg} ${styling.border} ${isSelected ? 'shadow-lg transform scale-105' : 'hover:shadow-md'}`}
      >
        ...
        <div className="w-full h-48 relative">
          ...
          <svg className="absolute inset-0 w-full h-full">
            {points.map((point, i) => (
              <circle 
                ...
                cy={point.y} 
                r={point.size} 
                className={`fill-${styling.starColor}`} 
              />
            ))}
          </svg>
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div className={`text-5xl opacity-20 ${isSelected ? 'text-white' : `text-${styling.starColor}`}`}>
              {zodiacSign.symbol}
            </div>
          </div>
        </div>
        ...
        <div className="p-3 bg-black/20 dark:bg-black/40">
          <div className="flex justify-between items-start">
            <div>
              <h3 className="font-bold text-white">{set.subject}</h3>
              <p className="text-xs text-white/80">{set.type}</p>
            </div>
            <div className="text-white/90 text-sm font-semibold bg-white/10 rounded-full h-7 w-7 flex items-center justify-center">
              {set.accuracy}%
            </div>
          </div>
  ...
  return (
    <div className="border border-slate-200 dark:border-slate-700 rounded-xl p-6 shadow-sm bg-gradient-to-b from-slate-900 to-indigo-950">
      <h2 className="text-xl font-bold mb-6 text-center text-white">Zodiac Constellations</h2>
      ...
      <div className="grid grid-cols-4 gap-2 mb-8">
        {['Fire', 'Earth', 'Air', 'Water'].map(element => {
          ...
          return (
            <div key={element} className={`${styling.bg} ${styling.border} rounded p-2 flex items-center justify-center`}>
              <div className={`w-3 h-3 rounded-full bg-${styling.starColor} mr-2`}></div>
              <span className="text-white text-sm">{element}</span>
            </div>
          );
        })}
      </div>
      ...
      <div className="space-y-6">
        {Object.entries(zodiacGroups).map(([zodiacName, zodiacSets]) => (
          <div key={zodiacName} className="border-t border-white/10 pt-4">
            <h3 className="text-white mb-3 font-medium flex items-center">
              <span className="text-lg mr-2">
                {zodiacSets[0]?.zodiacSign?.symbol}
              </span>
              <span>{zodiacName} ({zodiacSets.length})</span>
            </h3>
