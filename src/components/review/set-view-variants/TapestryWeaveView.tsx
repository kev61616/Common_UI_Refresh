...
export const TapestryWeaveView: React.FC<SetViewProps> = ({
  ...
  const generateWovenPattern = (set: any, isSelected: boolean) => {
          ...
          if (isHorizontal) {
            ...
            grid.push(
              <line
                ...
                y2={yPos + cellSize / 2}
                stroke={detailColor}
                strokeWidth={0.5} data-oid="vd_ny24" />
            ...
            grid.push(
              <line
                ...
                y2={yPos + cellSize}
                stroke={detailColor}
                strokeWidth={0.5} data-oid="q1hi4di" />
    ...
    return (
      <div
        ...
        onClick={() => onSelectSet(set.id)} data-oid="mw4-4dq">
        ...
        <svg
          ...
          <rect
            ...
            height={size}
            fill="#f8fafc"
            className="dark:fill-slate-800" data-oid="9cshhj." />
          ...
          <rect
            ...
            rx={8} data-oid="iz-sbkf" />

        </svg>
        
        <div className="mt-3 text-center" data-oid="i-cwr7b">
          <div className={`font-medium truncate ${isSelected ? 'text-primary' : ''}`} data-oid="lljjp9a">
            {set.title}
          </div>
          <div className="text-xs text-slate-500 dark:text-slate-400 mt-1" data-oid="zr:sowq">
  ...
  if (!mounted || isLoading) {
    return (
      <div className="min-h-[500px] flex items-center justify-center" data-oid="q.6hkua">
        <div className="w-8 h-8 border-t-2 border-primary rounded-full animate-spin" data-oid="15ew.xk"></div>
      </div>);
  ...
  if (!sets || sets.length === 0) {
    return (
      <div className="min-h-[500px] flex items-center justify-center bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-lg" data-oid="jttrxqh">
        <p className="text-slate-500 dark:text-slate-400" data-oid="n2ve8h8">No practice sets available to visualize</p>
      </div>);

  }

  return (
    <div className="border border-slate-200 dark:border-slate-700 rounded-xl p-4 shadow-sm" data-oid="7zlv5ii">
      <h3 className="text-xl font-bold mb-5 text-center" data-oid="mwjp.cn">Tapestry Weave View</h3>
      
      <div className="grid grid-cols-1 gap-8" data-oid="43bk79x">
        {Object.entries(groupedSets).map(([subject, subjectSets]) =>
        <div key={subject} data-oid="1661iia">
            <h4 className="text-lg font-medium mb-4 border-b pb-2 border-slate-200 dark:border-slate-700" data-oid="8zmnagf">
              {subject}
            </h4>
            
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6" data-oid="nl1k89u">
              {subjectSets.map((set) =>
            <div key={set.id} data-oid="y6djqzq">
      ...
      <div className="mt-6 border-t border-slate-200 dark:border-slate-700 pt-4" data-oid="iln04vh">
        <div className="text-center text-sm text-slate-500 dark:text-slate-400" data-oid=":w5_fpf">
          <span className="font-medium" data-oid="1dlsh.g">Tapestry Legend:</span>
          <span className="ml-2" data-oid="ku92_ic">Weave density represents question count | </span>
          <span data-oid="u2vh47p">Pattern complexity represents performance | </span>
          <span data-oid="sye13kl">Color schemes represent subjects</span>
        </div>
      </div>
    </div>);
