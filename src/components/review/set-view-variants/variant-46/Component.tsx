...
export function Component({ practiceSets, onSelectSet, selectedSetId }: SetViewProps) {
  ...
  const formatStarRating = (accuracy: number) => {
    ...
    return (
      <div className="flex" data-oid="i.r4r:3">
        {[...Array(5)].map((_, i) => {
          if (i < fullStars) {
            return (
              <svg key={i} className="w-4 h-4 text-yellow-300" fill="currentColor" viewBox="0 0 20 20" data-oid="26sj_0:">
                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" data-oid="qvs.4b4" />
              </svg>);

          } else if (i === fullStars && hasHalfStar) {
            return (
              <svg key={i} className="w-4 h-4 text-yellow-300" fill="currentColor" viewBox="0 0 20 20" data-oid="m8ghmog">
                <defs data-oid="8:b9ryf">
                  <linearGradient id={`halfStar-${accuracy}`} data-oid="i8.6g5w">
                    <stop offset="50%" stopColor="currentColor" data-oid="pk3pdn3" />
                    <stop offset="50%" stopColor="#CBD5E1" data-oid="qzf:qza" />
                  </linearGradient>
                </defs>
                <path fill={`url(#halfStar-${accuracy})`} d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" data-oid="kekjf60" />
              </svg>);

          } else {
            return (
              <svg key={i} className="w-4 h-4 text-gray-300 dark:text-gray-600" fill="currentColor" viewBox="0 0 20 20" data-oid="x.zbddp">
                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" data-oid="qv7-txa" />
              </svg>);
  ...
  return (
    <div className="bg-gray-950 text-white h-full overflow-auto p-6" data-oid=".ubzy4l">
      {/* Background "stars" */}
      <div className="absolute inset-0 overflow-hidden" data-oid="14nvoe9">
        {/* Static star field */}
        <div
      ...
      {/* Main content area */}
      <div className="relative z-10" data-oid="h___.nc">
        <div className="flex justify-between items-center mb-8" data-oid="2wgchkz">
          <h2 className="text-2xl font-bold font-mono" data-oid="58f0_hu">
            <span className="text-blue-400" data-oid="mm46f.3">★</span> Constellation Map <span className="text-blue-400" data-oid="ermje9o">★</span>
          </h2>
          
          {/* Legend for star types */}
          <div className="flex space-x-6" data-oid="a:5bjqi">
            <div className="flex items-center" data-oid="9d8-bpb">
              <div className="w-4 h-4 rounded-full bg-white shadow-white/60 mr-2" data-oid="64znp3r"></div>
              <span className="text-sm text-gray-300" data-oid="z62q4-o">Correct Answer</span>
            </div>
            <div className="flex items-center" data-oid="88sc853">
              <div className="w-3 h-3 rounded-full bg-white opacity-60 mr-2" data-oid="r1il9pq"></div>
              <span className="text-sm text-gray-300" data-oid="r509pye">Incorrect Answer</span>
            </div>
          </div>
        </div>
        
        {/* Galaxy regions by subject */}
        <div className="space-y-10" data-oid="sdy2ek9">
          {Object.entries(setsBySubject).map(([subject, sets]) => {
            const colors = getSubjectColors(subject);

            return (
              <div key={subject} className="relative" data-oid="hh2vl:u">
                {/* Subject header */}
                <div className="mb-4" data-oid="32wvbhn">
                  <h3 className={`text-xl font-mono ${colors.primary} border-b border-current pb-1`} data-oid="gka_t_8">
                ...
                <div
                  className={`absolute inset-0 -m-6 rounded-xl bg-gradient-to-b ${colors.region} -z-10`} data-oid="0j51d6n">
                </div>
                
                {/* Constellation sets grid */}
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8" data-oid="-w9krm:">
                  {sets.map((set) => {
                    ...
                    return (
                      <div
                        ...
                        onMouseLeave={() => setHoveredSetId(null)} data-oid="zrt0oxp">

                        {/* Visualization area */}
                        <div
                          className="relative h-60 overflow-hidden"
                          style={{ background: 'radial-gradient(circle at center, rgba(20, 24, 40, 0.7) 0%, rgba(10, 12, 24, 0.9) 100%)' }} data-oid="s5oj668">
                          ...
                          {/* Constellation lines */}
                          <svg className="absolute inset-0 w-full h-full" data-oid="5se:000">
                            {lines.map((line, i) =>
                            <line
                              key={i}
                              x1={line.x1}
                              ...
                              strokeDasharray={isSelected || isHovered ? "0" : "3,3"} data-oid="j909zr5" />
                          ...
                          {/* Selection indicator */}
                          {isSelected &&
                          <div className="absolute top-2 right-2 bg-white text-gray-900 rounded-full px-2 py-0.5 text-xs font-bold" data-oid=".yf:68j">
                              Selected
                            </div>
                          }
                        </div>
                        ...
                        <div className="p-4" data-oid="hy2zi3l">
                          {/* Set title and stats */}
                          <div className="flex justify-between items-center mb-2" data-oid="1i1n0u3">
                            <div data-oid="s:-5834">
                              <h4 className={`font-mono font-bold ${colors.primary}`} data-oid="3y2:p-t">{set.type}</h4>
                              <div className="text-xs text-gray-400" data-oid="qy5lzgd">
                                {set.questions.length} {set.questions.length === 1 ? 'star' : 'stars'} • {set.difficulty}
                              </div>
                            </div>
                            <div className="flex flex-col items-end" data-oid="n.2rf1a">
                              {formatStarRating(set.accuracy)}
                              <div className="text-xs text-gray-400 mt-1" data-oid="-g9au62">
                          ...
                          <div className="mt-2" data-oid="uax.o1o">
                            <div className="flex justify-between text-xs text-gray-400 mb-1" data-oid="qukhal2">
                              <span data-oid="8yga:_8">Progress</span>
                              <span data-oid="53g92pv">
                                {set.questions.filter((q) => q.answered).length}/{set.questions.length}
                              </span>
                            </div>
                            <div className="w-full h-1.5 bg-gray-800 rounded-full overflow-hidden" data-oid="73b3-r8">
                              <div
