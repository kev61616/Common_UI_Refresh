...
export const ArtStudioGalleryView: React.FC<SetViewProps> = ({ 
  ...
  if (!sets || sets.length === 0) {
    return (
      <div className="min-h-[500px] flex items-center justify-center bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-lg">
        <p className="text-slate-500 dark:text-slate-400">No artwork available in the studio</p>
      </div>
    )
  }
  ...
  const generateArtwork = (set: any, isSelected: boolean) => {
    ...
    const generateArtworkSVG = () => {
      ...
      const getCanvasBackground = () => {
        ...
        // Regular canvas
        return <rect x="0" y="0" width={size} height={size} fill={baseColor} />
      }
        ...
        <g>
          ...
          {accuracy > 85 && completionRatio > 0.8 && (
            <text
              x={size - 30}
              y={size - 10}
              className="text-[8px] fill-slate-600 dark:fill-slate-400 italic"
            >
              Student
            </text>
          )}
    ...
    return (
      <div className="relative flex flex-col h-full">
        {/* Art medium label */}
        <div className="absolute top-2 left-2 bg-white/80 dark:bg-black/50 text-xs px-2 py-0.5 rounded text-slate-600 dark:text-slate-300 font-medium capitalize">
          {medium}
        </div>
        ...
        >
          <div className="relative pb-[100%] w-full">
            <div className="absolute inset-0 bg-white dark:bg-slate-900 p-2">
              <svg 
                viewBox="0 0 180 180" 
          ...
          <div className="absolute inset-0 pointer-events-none">
            <div className="absolute inset-x-0 top-0 h-1 bg-slate-300 dark:bg-slate-600"></div>
            <div className="absolute inset-x-0 bottom-0 h-1 bg-slate-300 dark:bg-slate-600"></div>
            <div className="absolute inset-y-0 left-0 w-1 bg-slate-300 dark:bg-slate-600"></div>
            <div className="absolute inset-y-0 right-0 w-1 bg-slate-300 dark:bg-slate-600"></div>
          </div>
        </div>
        
        {/* Artwork title and info */}
        <div className="mt-3 text-center">
          <div className={`font-medium italic text-sm truncate ${isSelected ? 'text-primary' : ''}`}>
            "{set.title}"
          </div>
  ...
  return (
    <div className="border border-slate-200 dark:border-slate-700 rounded-xl p-4 shadow-sm">
      <h3 className="text-xl font-bold mb-4 text-center">Art Studio Gallery</h3>
      
      <div className="bg-slate-100 dark:bg-slate-800/50 p-4 md:p-6 rounded-lg">
        {/* Studio ambience elements */}
        <div className="border-b border-slate-200 dark:border-slate-700 pb-4 mb-6">
          <div className="flex items-center justify-between">
            <div className="text-sm text-slate-500 dark:text-slate-400">
              <span className="font-medium">Studio Collections</span> • {sets.length} works in progress
            </div>
        ...
        {/* Studio tools */}
        <div className="mt-8 border-t border-slate-200 dark:border-slate-700 pt-4 grid grid-cols-3 gap-4">
          <div className="bg-white dark:bg-black/20 rounded-lg p-3 flex flex-col items-center">
            <div className="text-xs font-medium text-slate-700 dark:text-slate-300 text-center">Art Style</div>
          ...
          <div className="bg-white dark:bg-black/20 rounded-lg p-3 flex flex-col items-center">
            <div className="text-xs font-medium text-slate-700 dark:text-slate-300 text-center">Medium</div>
            <div className="text-[10px] text-slate-500 dark:text-slate-400 mt-1 text-center">
          ...
          <div className="bg-white dark:bg-black/20 rounded-lg p-3 flex flex-col items-center">
            <div className="text-xs font-medium text-slate-700 dark:text-slate-300 text-center">Completion</div>
            <div className="text-[10px] text-slate-500 dark:text-slate-400 mt-1 text-center">
