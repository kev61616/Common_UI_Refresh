...
export const JazzCompositionView: React.FC<SetViewProps> = ({ 
  ...
  if (!sets || sets.length === 0) {
    return (
      <div className="min-h-[500px] flex items-center justify-center bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-lg">
        <p className="text-slate-500 dark:text-slate-400">No compositions available to display</p>
      </div>
    )
  }
  ...
  const generateNotes = (set: any, staffWidth: number) => {
    ...
    for (let i = 0; i < set.questions.length; i++) {
      ...
      // Note head
      notes.push(
        <g key={`note-${i}`} className="transition-all duration-300 hover:scale-110">
          <ellipse 
            cx={x} 
            cy={y} 
            rx={6} 
  ...
  const generateDecorations = (set: any, staffWidth: number) => {
    ...
    decorations.push(
      <text 
        ...
        x={35} 
        y={15} 
        className="fill-slate-800 dark:fill-slate-200 text-lg font-bold"
      >
        {beatsPerMeasure}
      </text>
    )
    ...
    if (set.pace === 'Fast') {
      decorations.push(
        <text 
          ...
          x={120} 
          y={-5} 
          className="fill-slate-800 dark:fill-slate-200 text-sm italic"
        >
          Allegro
        </text>
      )
  ...
  const renderComposition = (set: any, isSelected: boolean) => {
    ...
    return (
      <div
        className={`${
          isSelected ? 'ring-2 ring-indigo-500 dark:ring-indigo-400 shadow-lg' : ''
        } bg-amber-50 dark:bg-slate-800 rounded-xl p-4 shadow transition-shadow hover:shadow-md`}
      >
        <div className="flex justify-between items-center mb-3">
          <div>
            <h3 className="font-serif italic text-lg text-slate-800 dark:text-slate-200">{set.title}</h3>
            <div className="text-xs text-slate-500 dark:text-slate-400 mt-1 flex items-center">
              <span className="italic font-medium">{style}</span>
              <span className="mx-2">•</span>
              <span>{complexity}</span>
              <span className="mx-2">•</span>
              <span>Opus {set.id.slice(-2)}</span>
            </div>
          </div>
          
          <div className="text-right">
            <div className="font-medium text-sm text-indigo-600 dark:text-indigo-400">{set.accuracy || 0}% mastery</div>
        ...
        <div className="overflow-x-auto pb-2">
          <svg width={staffWidth} height={60} className="mx-auto">
    ...
    <div className="border border-slate-200 dark:border-slate-700 rounded-xl p-4 shadow-sm">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-lg p-4 shadow-inner">
          ...
          {Object.entries(groupedSets).map(([subject, subjectSets]) => (
            <div key={subject} className="mb-8">
              <h4 className="text-lg font-medium mb-4 border-b pb-2 border-slate-200 dark:border-slate-700 flex items-center">
                ...
                <span className="text-xs text-slate-500 dark:text-slate-400 font-normal italic">Album Collection</span>
              </h4>
        ...
        <div className="mt-8 border-t border-slate-200 dark:border-slate-700 pt-4">
          <h4 className="text-sm font-medium mb-3 text-center">Musical Notation Guide</h4>
          
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
            <div>
              <svg width="100" height="30" className="mx-auto">
                <g transform="translate(20, 15)">
                  <ellipse cx="10" cy="10" rx="6" ry="4" className="fill-emerald-500 dark:fill-emerald-400 stroke-slate-800 dark:stroke-slate-200" strokeWidth="1.5" />
                  <line x1="15" y1="10" x2="15" y2="-15" className="stroke-slate-800 dark:stroke-slate-200" strokeWidth="1.5" />
                </g>
                <text x="50" y="25" className="text-xs text-slate-600 dark:text-slate-400 text-center">Completed (High Accuracy)</text>
              </svg>
            </div>
            ...
            <div>
              <svg width="100" height="30" className="mx-auto">
                <g transform="translate(20, 15)">
                  <ellipse cx="10" cy="10" rx="6" ry="4" className="fill-none stroke-slate-800 dark:stroke-slate-200" strokeWidth="1.5" />
                  <line x1="15" y1="10" x2="15" y2="-15" className="stroke-slate-800 dark:stroke-slate-200" strokeWidth="1.5" />
                </g>
                <text x="50" y="25" className="text-xs text-slate-600 dark:text-slate-400 text-center">Not Completed</text>
              </svg>
            </div>
            ...
            <div>
              <svg width="100" height="30" className="mx-auto">
                <g transform="translate(20, 15)">
                  <line x1="0" y1="0" x2="30" y2="0" className="stroke-slate-800 dark:stroke-slate-200" strokeWidth="1" />
                  <line x1="0" y1="10" x2="30" y2="10" className="stroke-slate-800 dark:stroke-slate-200" strokeWidth="1" />
                </g>
                <text x="50" y="25" className="text-xs text-slate-600 dark:text-slate-400 text-center">Staff Lines</text>
              </svg>
            </div>
          </div>
        </div>
