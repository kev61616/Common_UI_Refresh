...
export function SetHeatmapView({ practiceSets, onSelectSet, selectedSetId }: SetViewProps) {
  ...
  return (
    <div className="border border-slate-200 dark:border-slate-700 rounded-xl p-6 shadow-sm bg-white dark:bg-slate-800">
      <h3 className="text-xl font-bold mb-6 text-center text-slate-700 dark:text-slate-200">Practice Set Performance Heatmap</h3>
      
      {/* Heatmap grid */}
      <div className="max-w-3xl mx-auto overflow-x-auto mb-6">
        <div className="min-w-[600px]">
          <div className="flex">
            {/* Y-axis label column */}
            <div className="w-36 pt-8 pr-3 text-right">
              <div className="font-medium text-slate-500 dark:text-slate-400 mb-2">
                Subject
              </div>
            </div>
            ...
            <div className="flex-1 grid grid-cols-3 gap-2 mb-2">
              {heatmapData.xAxis.map(label => (
                <div key={label} className="h-8 flex items-center justify-center font-medium text-slate-500 dark:text-slate-400">
                  {label}
                </div>
              ))}
            </div>
          ...
          <div className="space-y-2">
            {heatmapData.cells.map(row => (
              <div key={row.subject} className="flex">
                {/* Row label */}
                <div className="w-36 py-2 pr-3 text-right text-sm truncate flex items-center justify-end">
                  <div>
                    <div className="font-medium">{row.subject}</div>
                    <div className="text-xs text-slate-500 dark:text-slate-400">{row.totalCount} sets</div>
                  </div>
                </div>
                ...
                <div className="flex-1 grid grid-cols-3 gap-2">
                  {row.cells.map(cell => (
                    <div 
                      ...
                      onClick={() => selectCell(cell.subject, cell.difficulty)}
                    >
                      {cell.count > 0 ? (
                        <>
      ...
      {/* Legend */}
      <div className="flex justify-center items-center mb-8 text-xs space-x-2">
        <div className="font-medium text-slate-500 dark:text-slate-400 mr-1">Accuracy:</div>
        <div className="flex items-center">
          <div className="w-4 h-4 bg-red-100 dark:bg-red-900/30 rounded-sm"></div>
          <span className="ml-1">0-40%</span>
        </div>
        <div className="flex items-center">
          <div className="w-4 h-4 bg-amber-50 dark:bg-amber-900/20 rounded-sm"></div>
      ...
      {selectedDimension && selectedSets.length > 0 && (
        <div className="mt-8 p-6 bg-white dark:bg-slate-800/95 rounded-lg border border-slate-200 dark:border-slate-700 max-w-3xl mx-auto shadow-md">
          <h4 className="font-semibold mb-4 text-lg text-slate-700 dark:text-slate-200">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-6">
            <div className="p-3 bg-slate-50 dark:bg-slate-800/60 rounded-lg border border-slate-200 dark:border-slate-700">
              ...
              <div className="text-xl font-semibold text-slate-700 dark:text-slate-200">{selectedSets.length}</div>
            </div>
            <div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
              <div className="text-xs text-slate-500 dark:text-slate-400">Avg. Accuracy</div>
              <div className="text-xl font-semibold text-blue-600 dark:text-blue-400">{Math.round(selectedStats.avgAccuracy)}%</div>
            </div>
            <div className="p-3 bg-indigo-50 dark:bg-indigo-900/20 rounded-lg border border-indigo-200 dark:border-indigo-800">
              <div className="text-xs text-slate-500 dark:text-slate-400">Avg. Time</div>
              <div className="text-xl font-semibold text-indigo-600 dark:text-indigo-400">{formatTime(Math.round(selectedStats.avgTime))}</div>
            </div>
            <div className="p-3 bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-800">
              <div className="text-xs text-slate-500 dark:text-slate-400">Fast Sets</div>
              <div className="text-xl font-semibold text-green-600 dark:text-green-400">{selectedStats.fastSets}</div>
            </div>
            <div className="p-3 bg-amber-50 dark:bg-amber-900/20 rounded-lg border border-amber-200 dark:border-amber-800">
              <div className="text-xs text-slate-500 dark:text-slate-400">Slow Sets</div>
              <div className="text-xl font-semibold text-amber-600 dark:text-amber-400">{selectedStats.slowSets}</div>
            </div>
          </div>
          
          {/* Sets list */}
          <h5 className="font-medium mb-3 text-sm text-slate-600 dark:text-slate-300">Practice Sets</h5>
