...
export const TopicRow: React.FC<TopicRowProps> = ({
  ...
  return (
    <tr className="border-b border-slate-200 dark:border-slate-700">
      ...
      <td className="py-3 pr-4 font-medium text-slate-800 dark:text-slate-200 whitespace-nowrap">
        <div className="flex items-center gap-2">
          <span className="truncate max-w-[180px]" title={data.topic}>
            {data.topic}
          </span>
        </div>
      </td>
      ...
      {data.cells.map((cellData) => (
        <td key={`${data.topic}-${cellData.difficulty}`} className="p-1.5">
          <PerformanceCell 
            data={cellData}
            isSelected={
              !!selectedCell && 
