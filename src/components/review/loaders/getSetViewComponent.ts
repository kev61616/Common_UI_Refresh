...
/**
 * Dynamically loads a Set View component based on variant number
 * 
 ...
 */
export function getSetViewComponent(variant: number) {
  // Check if this variant is registered first
  if (!isViewRegistered('set', variant)) {
    console.warn(`Set view variant ${variant} is not registered. Using fallback component.`)
    return getFallbackComponent('set', variant)
  }
      ...
      import(`../set-view-variants/variant-${variant}/index`)
        ...
        .catch(error => {
          ...
          return import(`../set-view-variants/${getLegacyComponentName(variant)}`)
            ...
            .catch(error => {
              console.error(`Failed to load set view variant ${variant}:`, error)
              // If both fail, return the fallback
              return { default: getFallbackComponent('set', variant) }
            })
        })
    )
  } catch (error) {
    console.error(`Error setting up lazy loading for set view variant ${variant}:`, error)
    return getFallbackComponent('set', variant)
  }
}
...
function getLegacyComponentName(variant: number): string {
  ...
  const variantMap: Record<number, string> = {
    ...
    9: 'HexagonalView',
    10: 'MoodBasedView',
    11: 'FloatingIslandView',
    12: 'NeonArcadeView',
    13: 'GlassmorphismView',
    14: 'PaperCraftView',
    15: 'IsometricGridView',
    16: 'FrostedCrystalView',
    17: 'LayeredGlassView',
    18: 'BubbleGlassView',
    19: 'PrismaticGlassView',
    20: 'FramedGlassView',
    // Add more mappings as needed
  }

  return variantMap[variant] || `SetVariant${variant}View`
}
