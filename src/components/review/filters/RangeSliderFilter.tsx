...
export function RangeSliderFilter({
  ...
  return (
    <Dropdown
      ...
      elevated={true}
      className={`p-4 w-72 ${colorClasses.dropdown}`}
    >
      <div className="mb-4">
        ...
        <div className="text-xs font-medium text-slate-500 dark:text-slate-400 mb-2 mt-2">
          Custom Range
        </div>
        ...
        <div className="relative py-4">
          ...
          <div 
            ref={trackRef}
            className="w-full h-1 rounded-full bg-slate-200 dark:bg-slate-700"
          ></div>
          ...
          >
            ...
            <div className="absolute -inset-3 cursor-pointer" onMouseDown={() => setIsDragging('min')} onTouchStart={() => setIsDragging('min')}></div>
          </div>
          ...
          >
            {/* Enlarged touch target */}
            <div className="absolute -inset-3 cursor-pointer" onMouseDown={() => setIsDragging('max')} onTouchStart={() => setIsDragging('max')}></div>
          </div>
        </div>
        
        {/* Min-max guides */}
        <div className="flex justify-between mt-2">
          <span className="text-xs text-slate-500 dark:text-slate-400">{formatDisplayValue(min)}</span>
          <span className="text-xs text-slate-500 dark:text-slate-400">{formatDisplayValue(max)}</span>
        </div>
      </div>
      ...
      {(localRange[0] > min || localRange[1] < max) && (
        <button
          ...
          className="w-full p-2 text-xs font-medium text-center bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-md transition-colors dark:bg-slate-700 dark:hover:bg-slate-600 dark:text-slate-300"
        >
          Reset to Default
        </button>
      )}
