...
export const SeasonalCycleTimeline: React.FC<TimelineViewProps> = ({
  ...
  return (
    <div className="seasonal-cycle-timeline space-y-6 pb-8">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center space-x-3">
          ...
          <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-200">Seasonal Cycle Timeline</h2>
        </div>
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        {['Spring', 'Summer', 'Fall', 'Winter'].map(season => {
          ...
          return (
            <div 
              key={season}
              className={`rounded-lg p-3 ${styles.bg} border ${styles.border} flex items-center`}
            >
              <div className="mr-2">{styles.icon}</div>
              <div>
                <div className={`font-medium ${styles.text}`}>{season}</div>
      ...
      <div className="space-y-8">
        {Object.entries(yearlyGroups)
          ...
          .map(([year, seasons]) => (
            <div key={year} className="year-cycle">
              {/* Year header */}
              <div 
                className="year-header flex items-center space-x-2 cursor-pointer mb-4"
                onClick={() => setExpandedYear(expandedYear === year ? null : year)}
              >
                <div className={`w-6 h-6 rounded-full flex items-center justify-center ${expandedYear === year ? 'bg-amber-500 text-white' : 'bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300'}`}>
                  {expandedYear === year ? (
                ...
                <h3 className="text-xl font-semibold text-slate-800 dark:text-slate-200">{year} Cycle</h3>
                ...
                <div className="flex ml-auto space-x-2">
                  {Object.entries(seasons).map(([season, sets]) => {
                    ...
                    return (
                      <div 
                        key={season}
                        className={`px-2 py-0.5 rounded text-xs font-medium ${styles.bg} ${styles.text}`}
                      >
                        {sets.length} {season}
                      </div>
                    )
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
                    {['Winter', 'Spring', 'Summer', 'Fall'].map(season => {
                      ...
                      return (
                        <div 
                          key={season}
                          className={`season-card relative rounded-lg border overflow-hidden ${seasonStyles.border}`}
                        >
                          {/* Season header */}
                          <div className={`p-3 ${seasonStyles.bg} border-b ${seasonStyles.border} flex items-center`}>
                            {seasonStyles.icon}
                            <h4 className={`font-medium ml-2 ${seasonStyles.text}`}>{season}</h4>
                            <div className="ml-auto px-2 py-0.5 rounded-full bg-white/70 dark:bg-slate-800/70 text-xs font-medium text-slate-700 dark:text-slate-300">
                              {seasonSets.length} sessions
                            </div>
                          </div>
                          
                          {/* Season motif background */}
                          <div className={`absolute inset-0 opacity-5 pointer-events-none ${seasonStyles.motif}`}></div>
                          
                          {/* Session cards */}
                          <div className={`p-3 bg-white dark:bg-slate-800 ${seasonSets.length === 0 ? 'min-h-[100px]' : ''}`}>
                            {seasonSets.length === 0 ? (
                                    ...
                                    >
                                      <div className="p-3">
                                        <div className="flex items-start justify-between">
                                          <div>
                                            <h5 className={`font-medium ${subjectStyles.text}`}>
                                          ...
                                          <div className={`${subjectStyles.bg} text-white text-xs font-bold rounded-full h-6 w-6 flex items-center justify-center`}>
                                            {set.accuracy}
                                          </div>
                                        </div>
                                        ...
                                        <div className={`mt-2 text-xs italic ${seasonStyles.text}`}>
                                          ...
                                          <div className="mt-3 pt-3 border-t border-slate-200 dark:border-slate-700">
                                            <div className="grid grid-cols-3 gap-3 text-sm">
                                              <div>
                                                ...
                                                <div className="font-medium text-slate-700 dark:text-slate-300">{set.difficulty}</div>
                                              </div>
                                              <div>
                                                <div className="text-xs text-slate-500 dark:text-slate-400">Pace</div>
                                                <div className="font-medium text-slate-700 dark:text-slate-300">{set.pace}</div>
                                              </div>
                                              <div>
                                            ...
                                            {/* Mistake types */}
                                            <div className="mt-3">
                                              <div className="text-xs text-slate-500 dark:text-slate-400 mb-1">Challenges</div>
                                            ...
                                            {/* Performance trend */}
                                            <div className="mt-3">
                                              <div className="text-xs text-slate-500 dark:text-slate-400 mb-1">Performance Trend</div>
                                              <div className="relative h-2 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
                                                <div className="absolute top-0 left-0 bottom-0 w-1/3 bg-gradient-to-r from-slate-100 dark:from-slate-700 to-transparent z-10"></div>
                                                <div className="absolute top-0 bottom-0 left-1/3 w-1/3 bg-gradient-to-r from-transparent to-transparent z-10"></div>
                                                <div className="absolute top-0 right-0 bottom-0 w-1/3 bg-gradient-to-l from-slate-100 dark:from-slate-700 to-transparent z-10"></div>
                                            ...
                                            <div className="mt-3">
                                              <div className="flex justify-between text-xs">
                                                <span className="text-slate-500 dark:text-slate-400">Time Period</span>
                                                <span className="text-slate-700 dark:text-slate-300">{Math.floor(set.timeUsed / 60)} minutes</span>
                                              </div>
                                              <div className="flex justify-between text-xs mt-1">
                                                <span className="text-slate-500 dark:text-slate-400">Time of Day</span>
                                                <span className="text-slate-700 dark:text-slate-300">{set.timeOfDay}</span>
                                              </div>
                                            </div>
                                          </div>
                                        )}
