...
export function ParticlePhysicsTimeline({ practiceSets, onSelectSet, selectedSetId }: TimelineViewProps) {
  ...
  return (
    <div className="relative bg-slate-900 rounded-xl shadow-md p-6 overflow-hidden text-white">
      ...
      <h3 className="text-center text-2xl font-bold mb-6 text-white relative z-10">
        <span className="text-3xl bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-purple-500">30.</span> Particle Physics Timeline
      </h3>
      
      <div className="text-center mb-8 text-slate-400 font-mono text-sm">
          ...
          return (
            <div key={month} className="mb-16 last:mb-0">
              ...
              <div className="relative mb-8">
                <h4 className="text-xl font-bold text-center px-6 py-3 bg-slate-800 rounded-full inline-block mx-auto text-slate-200 border border-slate-700 relative z-10 shadow-lg">
                  <span className="opacity-70 mr-2 font-mono">T:</span>{month}
                </h4>
                
                {/* Energy field connector */}
                {monthIndex < sortedMonths.length - 1 && (
                  <div className="absolute left-1/2 top-full h-20 w-1 opacity-40 bg-gradient-to-b from-blue-500 via-purple-500 to-transparent"></div>
                )}
              </div>
                ...
                {sets.map((set, setIndex) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="absolute left-1/2 transform -translate-x-1/2 -translate-y-full top-0 opacity-75">
                        <div 
                      ...
                      <div 
                        ...
                        className={`relative mx-auto max-w-2xl rounded-lg cursor-pointer 
                                  ...
                                  transition-all shadow-lg overflow-hidden ${isSelected ? 'ring-2 ' + theme.ring : ''}`}
                      >
                        {/* Energy background */}
                        <div className={`absolute inset-0 opacity-10 bg-gradient-to-r ${theme.gradient}`}></div>
                        ...
                        <div className="absolute right-3 top-3 flex space-x-1">
                          {[1, 2, 3, 4, 5].map(level => (
                            <div 
                              key={level} 
                              className={`w-2 h-2 rounded-full ${level <= energyLevel ? theme.primary : 'bg-slate-700'}`}
                            ></div>
                          ))}
                        </div>
                        ...
                        <div className="flex items-center justify-between mb-4">
                          <div className="flex items-center">
                            ...
                            <div>
                              <h5 className={`font-bold ${theme.text}`}>{set.type}</h5>
                              <p className="text-xs text-slate-400">{theme.name} particle detected</p>
                            </div>
                          </div>
                          <div className="text-right">
                            <span className="text-xs text-slate-400 font-mono">{formatScientificDate(set.dateCompleted)}</span>
                          </div>
                        </div>
                        
                        {/* Subject and time info */}
                        <div className="flex flex-wrap gap-2 mb-3">
                          <span className={`px-2 py-1 text-xs rounded-full ${theme.light} ${theme.text}`}>
                            {set.subject}
                          </span>
                        ...
                        <div className="relative h-12 mb-3 bg-slate-900/50 rounded overflow-hidden flex items-center px-3">
                          <svg 
                          ...
                          <div className="absolute left-2 top-1 text-xs text-slate-500 font-mono">λ</div>
                          <div className="absolute right-2 text-xs text-slate-400">{set.accuracy}% accuracy</div>
                        </div>
                        
                        {/* Quantum Stats */}
                        <div className="grid grid-cols-2 gap-3 my-3">
                          <div className="bg-slate-900 rounded p-2 text-sm">
                            <div className="text-xs text-slate-500 mb-1">Spin States</div>
                            <div className="font-mono font-bold">
                              <span className="text-green-400">↑{set.questions.filter(q => q.correct).length}</span> / <span className="text-red-400">↓{set.questions.filter(q => !q.correct).length}</span>
                            </div>
                          </div>
                          <div className="bg-slate-900 rounded p-2 text-sm">
                            <div className="text-xs text-slate-500 mb-1">Error Coherence</div>
                            <div className="font-mono flex justify-between">
                              <span className="text-purple-400">C:{set.mistakeTypes.conceptual}</span>
                              <span className="text-amber-400">R:{set.mistakeTypes.careless}</span>
                            </div>
                          </div>
                        </div>
                        ...
                        {isSelected && (
                          <div className={`mt-4 pt-3 border-t border-slate-700 ${theme.text}`}>
                            <div className="text-sm">
                              <div className="font-bold mb-1">Quantum Observations</div>
      ...
      <div className="mt-8 p-4 bg-slate-800 rounded-lg border border-slate-700">
        <h5 className="text-center text-sm font-medium text-slate-300 mb-3">Particle Physics Legend</h5>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="flex items-center">
            <div className="h-6 w-6 rounded-full bg-blue-500 flex items-center justify-center text-white font-bold text-xs mr-2">γ</div>
            <span className="text-sm text-slate-300">Photon (Math)</span>
          </div>
          <div className="flex items-center">
            <div className="h-6 w-6 rounded-full bg-green-500 flex items-center justify-center text-white font-bold text-xs mr-2">e</div>
            <span className="text-sm text-slate-300">Electron (Reading)</span>
          </div>
          <div className="flex items-center">
            <div className="h-6 w-6 rounded-full bg-purple-500 flex items-center justify-center text-white font-bold text-xs mr-2">p</div>
            <span className="text-sm text-slate-300">Proton (Writing)</span>
          </div>
          <div className="flex items-center">
            <div className="h-6 w-6 rounded-full bg-amber-500 flex items-center justify-center text-white font-bold text-xs mr-2">n</div>
            <span className="text-sm text-slate-300">Neutron (Other)</span>
          </div>
        </div>
      </div>
