...
export const SpiralGalaxyTimeline: React.FC<TimelineViewProps> = ({
  ...
  const getAccuracyStars = (accuracy: number) => {
    ...
    for (let i = 0; i < 5; i++) {
      if (i < fullStars) {
        ...
        stars.push(
          <svg key={i} className="w-4 h-4 text-slate-500" fill="currentColor" viewBox="0 0 24 24">
            <path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"></path>
          </svg>
        )
      }
  ...
  return (
    <div ref={containerRef} className="spiral-galaxy-timeline w-full h-[700px] relative overflow-hidden">
      {/* Galaxy background */}
      ...
      <div className="absolute top-4 left-4 z-10">
        ...
        <p className="text-blue-200 mt-1 text-shadow-sm">
          Practice sets visualized as stars in a spiral galaxy
        </p>
      </div>
      
      {/* Legend */}
      <div className="absolute top-4 right-4 z-10 flex space-x-4 text-sm">
        <div className="flex items-center">
          <span className="inline-block w-3 h-3 rounded-full bg-blue-500 mr-2"></span>
          <span className="text-blue-200">Math</span>
        </div>
        <div className="flex items-center">
          <span className="inline-block w-3 h-3 rounded-full bg-emerald-500 mr-2"></span>
          <span className="text-blue-200">Reading</span>
        </div>
        <div className="flex items-center">
          <span className="inline-block w-3 h-3 rounded-full bg-amber-500 mr-2"></span>
          <span className="text-blue-200">Writing</span>
        </div>
      </div>
      
      {/* Galaxy center */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-20 h-20 rounded-full bg-white/5 flex items-center justify-center z-10">
        <div className="w-16 h-16 rounded-full bg-white/10 flex items-center justify-center">
          <div className="w-10 h-10 rounded-full bg-yellow-500/50 animate-pulse shadow-lg shadow-yellow-500/30 flex items-center justify-center">
            <div className="w-6 h-6 rounded-full bg-yellow-300/70 animate-pulse-slow"></div>
          </div>
        </div>
      </div>
      ...
      {sortedSets.map((set, index) => {
        ...
        return (
          ...
          >
            ...
            >
              <div className={`absolute inset-0 ${subjectColor} opacity-80 rounded-full`}></div>
              
              {/* Star rays - only show on hover/select */}
              {(isHovered || isSelected) && (
                <>
                  <div className="absolute inset-0 star-rays"></div>
                  <div className="absolute inset-0 star-pulse animate-pulse-slow"></div>
                </>
              )}
              ...
              <div className={`relative z-10 p-2 text-center transition-opacity duration-300
                ...
                <div className="text-xs font-semibold mb-1">{set.subject}</div>
                <div className="text-[10px]">{set.type}</div>
            ...
            <div className={`absolute -bottom-6 left-1/2 -translate-x-1/2 text-center transition-opacity duration-300
              ${isHovered || isSelected ? 'opacity-100' : 'opacity-0'}`}>
              <span className="text-xs text-blue-200 whitespace-nowrap">{formatDate(set.dateCompleted)}</span>
            </div>
            
            {/* Detail card on selection */}
            {isSelected && (
              <div className="absolute top-full left-1/2 -translate-x-1/2 mt-4 w-64 max-w-xs
                bg-slate-800/80 backdrop-blur-sm border border-slate-700 rounded-lg p-4 shadow-xl z-40">
                <h4 className="text-white font-medium">{set.subject}: {set.type}</h4>
                <div className="text-xs text-blue-200 mt-1">{formatDate(set.dateCompleted)}</div>
                ...
                <div className="grid grid-cols-2 gap-2 mt-3">
                  ...
                  <div className="bg-slate-900/50 rounded p-2">
                    ...
                    <div className="text-white font-medium">{set.difficulty}</div>
                  </div>
                  <div className="bg-slate-900/50 rounded p-2">
                ...
                <div className="mt-3 bg-slate-900/50 rounded p-2">
                  <div className="text-[10px] text-slate-400 mb-1">Performance Trends</div>
                  <div className="flex items-end h-12 space-x-1">
                    <div className="h-full flex flex-col justify-end flex-1">
                      <div className="text-[8px] text-slate-500 text-center">Early</div>
                      <div 
                    ...
                    <div className="h-full flex flex-col justify-end flex-1">
                      <div 
                        ...
                        style={{ height: `${(set.sessionFatigue.earlyAccuracy + set.sessionFatigue.lateAccuracy) / 2 * 0.75}%` }}
                      ></div>
                    </div>
                    <div className="h-full flex flex-col justify-end flex-1">
                      <div 
                    ...
                    <div className="h-full flex flex-col justify-end flex-1">
                      <div className="text-[8px] text-slate-500 text-center">Late</div>
                      <div 
                        className="bg-blue-500/50 rounded-t-sm w-full"
                        style={{ height: `${set.sessionFatigue.lateAccuracy * 0.65}%` }}
                      ></div>
                    </div>
                  </div>
                </div>
