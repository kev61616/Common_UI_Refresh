      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {currentItems.length > 0 ? (
          currentItems.map((set) => (
            ...
            >
              {/* Top gradient banner */}
              <div className={`h-24 w-full bg-gradient-to-r ${getSubjectGradient(set.subject)} flex items-center justify-center relative`}>
                ...
                {/* Difficulty badge */}
                <div className="absolute top-2 right-2 bg-white/30 backdrop-blur-sm text-white px-3 py-1 rounded-full text-xs font-semibold">
                  {set.difficulty}
                </div>
                
                {/* Date badge */}
                <div className="absolute bottom-2 left-2 bg-black/20 backdrop-blur-sm text-white px-3 py-1 rounded-full text-xs">
                  {formatDate(set.dateCompleted)}
                </div>
                
                {/* Time badge */}
                <div className="absolute bottom-2 right-2 bg-black/20 backdrop-blur-sm text-white px-3 py-1 rounded-full text-xs">
                  {set.timeOfDay}
                </div>
              </div>
              ...
              {/* Content */}
              <div className="p-5">
                <h3 className="text-xl font-bold text-slate-800 dark:text-white flex justify-between items-center">
                  <span>{set.subject}</span>
                  <span className="text-slate-500 dark:text-slate-400 text-sm font-normal">{set.type}</span>
                </h3>
                ...
                <div className="mt-4 flex items-stretch">
                  ...
                  <div className="w-1/3 flex flex-col items-center">
                    ...
                    <span className="text-xs text-slate-500 dark:text-slate-400 mt-1">Accuracy</span>
                  </div>
                  
                  {/* Time */}
                  <div className="w-1/3 flex flex-col items-center justify-center border-x border-slate-100 dark:border-slate-800">
                  ...
                  <div className="w-1/3 flex flex-col items-center justify-center">
                    ...
                    <span className="text-xs text-slate-500 dark:text-slate-400 mt-1">Pace</span>
                  </div>
                </div>
                ...
                <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-800">
                  <div className="flex flex-wrap gap-2">
                    {set.mistakeTypes.conceptual > 0 && (
                      <span className="px-2 py-1 bg-indigo-50 text-indigo-700 text-xs rounded-md dark:bg-indigo-900/30 dark:text-indigo-300">
                        {set.mistakeTypes.conceptual} Conceptual
                      </span>
                    )}
                    {set.mistakeTypes.careless > 0 && (
                      <span className="px-2 py-1 bg-amber-50 text-amber-700 text-xs rounded-md dark:bg-amber-900/30 dark:text-amber-300">
                        {set.mistakeTypes.careless} Careless
                      </span>
                    )}
                    {set.mistakeTypes.timeManagement > 0 && (
                      <span className="px-2 py-1 bg-red-50 text-red-700 text-xs rounded-md dark:bg-red-900/30 dark:text-red-300">
                        {set.mistakeTypes.timeManagement} Time
                      </span>
                    )}
                    {set.sessionFatigue.earlyAccuracy - set.sessionFatigue.lateAccuracy > 15 && (
                      <span className="px-2 py-1 bg-purple-50 text-purple-700 text-xs rounded-md dark:bg-purple-900/30 dark:text-purple-300">
        ...
        <div className="flex justify-center mt-8">
          <nav className="inline-flex rounded-xl shadow-md overflow-hidden">
            <button
              ...
              className="relative inline-flex items-center px-4 py-2.5 bg-white/90 backdrop-blur-sm text-sm font-medium text-slate-600 hover:bg-white transition-colors duration-200 disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-white/90 dark:bg-slate-800/90 dark:text-slate-300 dark:hover:bg-slate-800"
            >
