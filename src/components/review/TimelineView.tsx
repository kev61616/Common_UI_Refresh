...
export function TimelineView({ practiceSets, onSelectSet, selectedSetId, sortConfig, onSortChange }: TimelineViewProps) {
  ...
  return (
    ...
    >
        ...
        <div className="flex space-x-1.5">
          {chapters.map((chapter, index) => {
            ...
            return (
              ...
              >
                <span className={`mr-2 transition-transform duration-300 ${isActive ? 'transform scale-125' : ''}`}>
      ...
      <div className="relative pb-10 pt-4 ml-4 md:ml-8">
        {/* Decorative timeline path with gradient */}
        <div className="absolute top-0 bottom-0 left-0 w-1 md:w-1.5 bg-gradient-to-b from-indigo-200 via-indigo-300 to-indigo-500 dark:from-indigo-900 dark:via-indigo-800 dark:to-indigo-600 rounded-full"></div>
          ...
          return (
            ...
            >
              ...
              <div className={`absolute -left-3 md:-left-4 w-6 h-6 md:w-8 md:h-8 
                           ...
                           ${animationEnabled ? 'animate-pulse-subtle' : ''}`}>
                <span className="text-xs md:text-sm font-bold text-indigo-600 dark:text-indigo-300">
                  {chapterIndex + 1}
                </span>
              </div>
              
              {/* Enhanced chapter header with card styling */}
              <div className="ml-6 md:ml-10 mb-6 p-3 bg-white/50 dark:bg-slate-800/20 rounded-lg border border-slate-100 dark:border-slate-700/40 backdrop-blur-sm">
                <h5 className="text-xl font-bold flex items-center text-indigo-600 dark:text-indigo-400 gap-2">
                  {chapter.title} 
                  <span className={`${animationEnabled && isLoaded ? 'animate-bounce-subtle' : ''}`}>
                    {getMilestoneEmoji(avgAccuracy)}
                  </span>
                </h5>
              ...
              <div className="space-y-6 ml-6 md:ml-10">
                    ...
                    <div 
                      ...
                      className={`p-4 rounded-lg cursor-pointer border transition-all duration-300
                                ...
                                ${selectedSetId === set.id 
                                  ...
                                  : 'bg-white border-slate-200 dark:bg-slate-800 dark:border-slate-700 hover:shadow-md hover:scale-[1.01] hover:bg-slate-50/80 dark:hover:bg-slate-700/70'}`}
                    >
                      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                        ...
                        <div className="flex-grow">
                          ...
                          {isExpanded && (
                            <div className={`mt-4 pt-3 border-t border-slate-100 dark:border-slate-700 ${
                              ...
                              <h4 className="font-medium text-sm text-slate-700 dark:text-slate-300 mb-2">Detailed Performance</h4>
                              <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                                <div className="text-xs bg-slate-50 dark:bg-slate-800/80 p-2 rounded">
                        ...
                        <div className="flex flex-row md:flex-col items-center gap-3 md:min-w-[100px]">
                          <div className={`w-16 h-16 rounded-full flex items-center justify-center transition-all duration-300 shadow-md ${
                            ...
                            <span className={`text-2xl font-bold ${animationEnabled && (isHovered || selectedSetId === set.id) ? 'animate-pulse-subtle' : ''}`}>
                              {set.accuracy}%
                            </span>
                          </div>
                          ...
                          <div className="flex flex-wrap gap-1 justify-center">
                            {set.id === highestAccuracy.id && (
                              <span className="bg-gradient-to-r from-indigo-100 to-purple-100 dark:from-indigo-900/30 dark:to-purple-900/20 text-indigo-700 dark:text-indigo-300 text-xs px-2 py-1 rounded-full shadow-sm flex items-center gap-1">
                                <span>⭐</span>
                                <span>Highest</span>
                              </span>
                            )}
                            {set.id === fastestSet.id && (
                              <span className="bg-gradient-to-r from-sky-100 to-blue-100 dark:from-sky-900/30 dark:to-blue-900/20 text-sky-700 dark:text-sky-300 text-xs px-2 py-1 rounded-full shadow-sm flex items-center gap-1">
                                <span>⚡</span> 
                                <span>Fastest</span>
                              </span>
                            )}
                ...
                <div className="p-4 bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-800/50 dark:to-slate-800/30 rounded-lg border border-slate-200 dark:border-slate-700 mt-4 shadow-md">
                  <div className="font-medium text-slate-700 dark:text-slate-300">Chapter Summary</div>
        ...
        <div className={`absolute -left-3 md:-left-4 bottom-0 w-6 h-6 md:w-8 md:h-8 
                      ...
                      ${animationEnabled ? 'animate-pulse-subtle' : ''}`}>
          <span className="text-white transform rotate-0">🏁</span>
        </div>
        <div className="ml-6 md:ml-10 pb-6">
          <div className="text-lg font-bold text-indigo-600 dark:text-indigo-400 flex items-center gap-2">
            Current Progress
            <span className={`text-sm inline-block ${animationEnabled ? 'animate-bounce-subtle' : ''}`}>
              👨‍🎓
            </span>
          </div>
          ...
          {displaySets.length > 0 && (
            <div className="mt-3 bg-slate-100 dark:bg-slate-700/30 h-2 w-full rounded-full overflow-hidden">
              <div 
