...
export function Dialog({
  ...
  'as' | 'className'
>) {
  // Destructure unmount from props to avoid TypeScript error
  const { unmount, ...dialogProps } = props
  return (
    <Headless.Dialog {...dialogProps}>
        ...
        <div className="grid min-h-full grid-rows-[1fr_auto] justify-items-center sm:grid-rows-[1fr_auto_3fr] sm:p-4">
          <Headless.DialogPanel
            ...
            className={clsx(
              className,
              sizes[size],
              'row-start-2 w-full min-w-0 rounded-t-3xl bg-white p-8 ring-1 shadow-lg ring-zinc-950/10 sm:mb-auto sm:rounded-2xl dark:bg-zinc-900 dark:ring-white/10 forced-colors:outline',
